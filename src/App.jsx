...
function BiometricScan({ palmImage, palmLandmarks, onComplete }) {
  ...
  // Pre-load audio once so it plays instantly with no gap on loop
  useEffect(() => {
    try {
      ...
      a.preload = "auto";
      buzzRef.current = a;
      buzzReadyRef.current = true;
    } catch(e) {}
    return () => {
      try { if (buzzRef.current) { buzzRef.current.pause(); } } catch(e) {}
    };
  }, []);

  const startBuzz = () => {
    try {
      const a = buzzRef.current;
      if (!a) return;
      a.currentTime = 0;
      a.play().catch(() => {});
    } catch(e) {}
