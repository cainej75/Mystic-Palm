...
export default function MysticFortunes() {
  ...
  useEffect(()=>{
    if(phase==="biometric" && palmImage) {
      // If no landmarks were captured, reject immediately
      if (!palmLandmarks) {
        console.warn("User: No landmarks detected - rejecting image");
        setPalmError(true);
        setPhase("capture");
        return;
      }
  ...
  useEffect(()=>{
    if(partnerPhase==="biometric" && partnerPalmImage) {
      // If no landmarks were captured, reject immediately
      if (!partnerPalmLandmarks) {
        console.warn("Partner: No landmarks detected - rejecting image");
  ...
  const validatePalmImage = (lm) => {
    try {
      // Must have landmarks - if missing, it's not a hand
      if (!lm || lm.length < 21) return false;
      
      const wrist = lm[0], indexMCP = lm[5], pinkyMCP = lm[17];

      // Left hand check - thumb must be on left side
      const thumbLeft = lm[4].x < lm[20].x;
      if (!thumbLeft) return false;

      // Palm facing: cross product - must be strong positive
      const v1x = indexMCP.x - wrist.x, v1y = indexMCP.y - wrist.y;
      const v2x = pinkyMCP.x - wrist.x, v2y = pinkyMCP.y - wrist.y;
      const cross = v1x * v2y - v1y * v2x;
      if (cross <= 0.01) return false;  // Must be clearly facing forward

      // Fingers open - all tips must be clearly above their MCPs
      const fingersOpen = [8,12,16,20].every(tip => lm[tip].y < lm[tip-2].y);
      if (!fingersOpen) return false;
      ...
      const handCentred  = cx > 0.25 && cx < 0.75;
      const wristLow     = wristY > 0.55;           // Wrist at bottom
      const tipsHigh     = midTipY < 0.55;          // Tips at top
      const bigEnough    = handSpanX > 0.32 && handSpanY > 0.35;  // Hand fills frame
      const notTooHigh   = cy < 0.80;               // Not off top of screen
...
Respond ONLY with valid JSON, no markdown: {"greeting":"dramatic 1-sentence max 18 words","teaser":"2-3 sentence teaser with hook","lifeLine":"2 vivid sentences","heartLine":"2 vivid sentences","headLine":"2 vivid sentences","fateLine":"2 vivid sentences","fullReading":"4 paragraphs with \\n\\n","luckyNumber":"1-9","luckyColor":"evocative colour","soulSign":"mystical archetype 2-4 words","warning":"one ominous sentence","gift":"one sentence","pastLife":"one past life hint","nearFuture":"one 90-day prediction"}`;
    // Hand was already validated during capture phase, no need to validate again
    
    // Step 1: Always generate seed fortune first
    const _seed = generateSeed(userName, birthDate);
    const seedFortune = generateFortune(_seed, userName, birthDate);

    // Step 2: Get Claude reading and merge seed values in
    try{
      const res=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},
