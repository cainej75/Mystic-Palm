  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (lm) {
        ...
        // Left hand + palm facing — works on both mirrored and non-mirrored cameras
        // Mirrored camera (most phones): left palm = label 'Right' AND cross > 0
        // Non-mirrored camera (Samsung/some Android): left palm = label 'Left' AND cross < 0
        const label = lastResultsRef.current?.multiHandedness?.[0]?.label;
        const v1x = indexMCP.x - wrist.x, v1y = indexMCP.y - wrist.y;
        const v2x = pinkyMCP.x - wrist.x, v2y = pinkyMCP.y - wrist.y;
        const cross = v1x * v2y - v1y * v2x;
        const mirroredLeftPalm    = label === "Right" && cross > 0.02;
        const nonMirroredLeftPalm = label === "Left"  && cross < -0.02;
        const thumbRight = lm[4].x > lm[20].x;  // mirrored fallback
        const thumbLeft  = lm[4].x < lm[20].x;  // non-mirrored fallback
        const fallbackMirrored    = !label && thumbRight && cross > 0.02;
        const fallbackNonMirrored = !label && thumbLeft  && cross < -0.02;
        const isLeftHand = mirroredLeftPalm || nonMirroredLeftPalm || fallbackMirrored || fallbackNonMirrored;

        // Palm facing: cross product wrist→indexMCP × wrist→pinkyMCP
        const palmFacing = isLeftHand; // palmFacing is implied by the isLeftHand check above
