...
export default function MysticFortunes() {
    ...
    const verifyPaymentOnReturn = async () => {
      ...
      if (sessionId.startsWith('free_')) {
        ...
        if (productType === 'full_revelation') {
          ...
          // Restore palmImage for Claude enhancement
          const savedPalmImage = localStorage.getItem('pendingPalmImage');
          if (savedPalmImage) {
  ...
  useEffect(() => {
    if (fullPaid && !subscribed && reading && palmImage && phase !== "loading") {
      console.log("🎯 Payment confirmed, enhancing reading with Claude...");
  ...
  // PAID FEATURE: Enhance the reading with Claude's theatrical flair
  const enhanceReadingWithClaude = async () => {
    if (!reading || !palmImage) return;
    
    console.log("✨ Enhancing reading with Claude (Full Revelation)...");
    setPhase("loading");
    ...
    try {
      ...
      const mt = "image/jpeg";
      const prompt = `You are Madame Zafira, an ancient and highly perceptive palm reader. You do not speak like an AI or assistant. You speak with calm authority and quiet certainty. You never express uncertainty.

Analyze the palm in the image and deliver a deeply personal and specific reading for ${userName||"Seeker"}.

RULES:
- Refer to specific palm features: life line, heart line, head line, breaks, forks, or markings
- Speak as if you can clearly see these details
- Connect features to their past, present struggles, and future path
- Include one emotionally impactful, deeply personal statement
- Include one specific near-future turning point prediction
- Include one statement that feels true but rarely admitted to others
- Avoid vague language like "may" or "could" — speak with absolute confidence
- Use their name when delivering insights
- Keep tone mystical, slightly eerie, but ultimately reassuring
- Do not mention uncertainty. Do not say you are guessing. Deliver as undeniably true.

Respond ONLY with valid JSON, no markdown: {"greeting":"one powerful opening statement about what you see in the palm","teaser":"immediate hook about their life right now","lifeLine":"specific analysis of their life line - past, present trajectory","heartLine":"what their heart line reveals about love and vulnerability","headLine":"their intellectual pattern and internal conflict","fateLine":"their direction and choice point ahead","fullReading":"4-5 paragraphs weaving together: a specific past event their palm shows, their current internal struggle, their near-future turning point, and one deeply personal truth they know but won't admit","luckyNumber":"1-9","luckyColor":"a color name that feels personally resonant","soulSign":"a mystical archetype in 2-4 words that captures their essence","warning":"one important caution about the path ahead","gift":"their hidden strength or gift","pastLife":"one vivid detail about who they were before","nearFuture":"one specific prediction for the next 90 days"}`;
      ...
      // Merge Claude's theatrical writing with existing algorithm attributes
      setReading({
        ...reading, // Keep existing reading
        ...claudeR, // Override with Claude's theatrical content
        soulSign: reading.soulSign,        // Keep algorithm's attributes
        luckyColor: reading.luckyColor,
        luckyNumber: reading.luckyNumber,
        personality: reading.personality,
        pastLife: reading.pastLife,
        gift: reading.gift,
        warning: claudeR.warning || reading.warning,
      });
      console.log("✅ Reading enhanced with Claude!");
      
    } catch(e) {
      console.error("Claude enhancement failed:", e);
      // Keep existing reading if Claude fails
    }
    
    // Clean up localStorage
    try { localStorage.removeItem('pendingPalmImage'); } catch(e) {}
  ...
  const handleFullPayment = () => {
    ...
    try { if (palmImage) localStorage.setItem('pendingPalmImage', palmImage); } catch(e) {}
