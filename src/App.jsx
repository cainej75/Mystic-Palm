...
const ZAFIRA_RESULT_VIDEO = "/Madame_Zafira_Result.mp4";
          ...
          <>
            <video
              ...
              style={{
                ...
                height:"100%",
                objectFit:"cover",
                transform:"translate(-48%, -50%) scale(1.18)",
                display:"block",
            ...
            {/* Overlay: play button when stopped, tap-to-stop area when playing */}
            <div
              onClick={()=>{
                const v = videoRef && videoRef.current;
                if (!v) return;
...
function DesktopGate() {
  const [bypassed, setBypassed] = React.useState(()=>{
    try { return sessionStorage.getItem('devDesktop')==='1'; } catch(e){ return false; }
  });
  const [keySeq, setKeySeq] = React.useState('');

  React.useEffect(()=>{
    const handler = (e)=>{
      const next = (keySeq + e.key).slice(-4);
      setKeySeq(next);
      if (next === '6969') {
        try { sessionStorage.setItem('devDesktop','1'); } catch(e){}
        setBypassed(true);
      }
    };
  ...
  return (
    <div style={{
      ...
      ['@media(minWidth:769px)' as any]: {display:'flex'},
    }} className="desktop-gate">
      <style>{`
        @media (min-width: 769px) { .desktop-gate { display: flex !important; } }
        @media (max-width: 768px) { .desktop-gate { display: none !important; } }
        @keyframes desktopAura{0%,100%{opacity:0.5;transform:scale(1)}50%{opacity:1;transform:scale(1.08)}}
        @keyframes desktopFade{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
      `}</style>

      {/* Particle shimmer bg */}
      <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 30%, rgba(96,42,120,0.18) 0%, transparent 70%)',pointerEvents:'none'}}/>
      <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 80%, rgba(176,64,90,0.1) 0%, transparent 60%)',pointerEvents:'none'}}/>

      {/* Crystal ball glyph */}
      <div style={{fontSize:72,marginBottom:28,animation:'desktopAura 4s ease-in-out infinite',filter:'drop-shadow(0 0 24px rgba(201,168,76,0.6))'}}>🔮</div>

      {/* Title */}
      <div style={{fontFamily:'Cinzel,serif',fontSize:'clamp(28px,3vw,42px)',fontWeight:700,color:'#c9a84c',letterSpacing:6,textTransform:'uppercase',textAlign:'center',marginBottom:10,animation:'desktopFade 1s ease both',filter:'drop-shadow(0 0 12px rgba(201,168,76,0.5))'}}>
        Mystic Fortunes
      </div>

      {/* Gold divider */}
      <div style={{width:120,height:1,background:'linear-gradient(90deg,transparent,#c9a84c,transparent)',margin:'0 auto 32px',animation:'desktopFade 1s ease 0.2s both'}}/>

      {/* Message */}
      <div style={{maxWidth:480,textAlign:'center',padding:'0 32px',animation:'desktopFade 1s ease 0.4s both'}}>
        <p style={{fontFamily:'IM Fell English,serif',fontStyle:'italic',fontSize:'clamp(18px,1.8vw,24px)',color:'#e8d5b8',lineHeight:1.9,margin:'0 0 20px',opacity:0.92}}>
          "The ancient lines of the palm can only be read by candlelight —
          and candlelight lives in your hand."
        </p>
        <p style={{fontFamily:'Cinzel,serif',fontSize:'clamp(12px,1.1vw,15px)',color:'rgba(201,168,76,0.7)',letterSpacing:3,textTransform:'uppercase',margin:0}}>
          This experience was crafted for mobile
        </p>
        <p style={{fontFamily:'Cinzel,serif',fontSize:'clamp(12px,1.1vw,15px)',color:'rgba(201,168,76,0.5)',letterSpacing:2,textTransform:'uppercase',margin:'8px 0 0'}}>
          Open it on your phone to begin your reading
        </p>
      </div>

      {/* Bottom ornament */}
      <div style={{marginTop:40,animation:'desktopFade 1s ease 0.6s both',opacity:0.4,fontFamily:'Cinzel,serif',fontSize:18,color:'#c9a84c',letterSpacing:8}}>✦ ✦ ✦</div>
    </div>
  );
}
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"0 16px 24px",position:"relative",zIndex:2}}>
        ...
        {phase==="result"&&reading&&(
          <div style={{paddingTop:28,paddingBottom:20,animation:"fadeUp 0.6s ease both"}}>
            ...
            {(()=>{
              const resultVidRef = videoRef2;
              return (
                <div style={{position:"relative",width:"100%",margin:"0 auto"}} onContextMenu={e=>e.preventDefault()}>
                  {/* Outer aura */}
                  <div style={{position:"absolute",inset:-4,borderRadius:20,boxShadow:`0 0 60px ${C.gold}55, 0 0 120px ${C.gold}22`,animation:"auraPulse 3s ease-in-out infinite",pointerEvents:"none",zIndex:0}}/>
                  {/* Main video frame */}
                  <div style={{position:"relative",borderRadius:18,overflow:"hidden",border:`2px solid ${C.gold}88`,boxShadow:`0 12px 48px rgba(0,0,0,0.7), inset 0 0 40px rgba(0,0,0,0.4)`,zIndex:1,width:"100%",paddingTop:"133.33%",background:"#000"}}>
                    <video
                      ref={resultVidRef}
                      src={ZAFIRA_RESULT_VIDEO}
                      playsInline
                      controls={false}
                      disableRemotePlayback
                      disablePictureInPicture
                      x-webkit-airplay="deny"
                      webkit-playsinline="true"
                      style={{position:"absolute",top:"50%",left:"50%",width:"100%",height:"100%",objectFit:"cover",transform:"translate(-54%, -56%) scale(1.32)",display:"block",WebkitMediaControlsPanel:"none",pointerEvents:muted?"none":"auto",cursor:muted?"default":"pointer"}}
                      onClick={()=>{
                        const v=resultVidRef&&resultVidRef.current;
                        if(!v)return;
                        v.pause();v.currentTime=0;v.onended=null;setMuted(true);
                      }}
                    />
                    ...
                    <div
                      onClick={()=>{
                        const v=resultVidRef&&resultVidRef.current;
                        if(!v)return;
                        if(muted){v.muted=false;v.currentTime=0;v.play();setMuted(false);v.onended=()=>{v.pause();v.currentTime=0;setMuted(true);};}
                      }}
                      style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",cursor:muted?"pointer":"default",zIndex:4,background:"transparent",pointerEvents:muted?"auto":"none"}}>
                      {muted&&(
                        <div style={{width:64,height:64,borderRadius:"50%",background:"rgba(176,64,90,0.85)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 24px rgba(176,64,90,0.7), 0 0 48px rgba(176,64,90,0.3)",animation:"introPulse 2s ease-in-out infinite"}}>
                          <span style={{fontSize:26,marginLeft:4,color:"white"}}>&#9654;</span>
                        </div>
                      )}
                    </div>
                    {/* Vignette */}
                    <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at center, transparent 40%, rgba(8,5,16,0.55) 100%)",pointerEvents:"none"}}/>
                    {/* Name plate */}
                    <div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent, rgba(8,5,16,0.95))",padding:"32px 16px 14px",textAlign:"center",pointerEvents:"none"}}>
                      <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.gold,letterSpacing:4,textTransform:"uppercase"}}>Madame Zafira</div>
                      <div style={{width:60,height:1,background:`linear-gradient(90deg,transparent,${C.gold},transparent)`,margin:"6px auto 0"}}/>
                    </div>
                  </div>
                </div>
              );
