...
// ── PALM LINE DESCRIPTION ARRAYS ── (used by algorithm reading for unique section content)
const PALM_LIFE_LINES = [
...
const FullScreenShareCard = React.memo(function FullScreenShareCard({reading, userName, birthDate, onClose, todayFortune}) {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: 'My Mystic Fortune Reading', text: 'Check out my reading from mysticfortunes.ai' });
        } else {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"linear-gradient(160deg,rgba(8,3,20,0.98) 0%,rgba(40,8,25,0.98) 50%,rgba(8,3,20,0.98) 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:1000,overflowY:"auto"}}>
      ...
      <button onClick={handleShare} disabled={sharing} style={{marginTop:20,background:sharing?"#3a1a22":"linear-gradient(135deg,#b0405a,#5c1a2a)",border:"none",color:"white",borderRadius:10,padding:"12px 30px",fontSize:12,fontFamily:"Cinzel,serif",fontWeight:600,cursor:sharing?"not-allowed":"pointer",letterSpacing:1,textTransform:"uppercase",opacity:sharing?0.7:1}}>
...
const FullscreenCamera = React.memo(function FullscreenCamera({ canvasRef, onCapture, onCancel }) {
  ...
  const [msg, setMsg]           = useState("Place your LEFT hand in front of the rear camera, palm facing you");
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      let isValid = false;
      let newMsg = "Place your LEFT hand in front of the rear camera, palm facing you";
      ...
      if (lm) {
        ...
        newMsg = isValid ? "Hold steady…" : "Place your LEFT hand in front of the rear camera, palm facing you";
      } else if (mpStatus === "loading") {
        newMsg = "Please wait while the hand detection is loading…";
      } else {
        newMsg = "Place your LEFT hand in front of the rear camera, palm facing you";
      }
  ...
  const shareConnection = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        const file = new File([blob], "cosmic-connection.png", {type:"image/png"});
        if (navigator.canShare && navigator.canShare({files:[file]})) {
          await navigator.share({title:"Our Cosmic Connection", text:"We scored " + score + "% compatibility on MysticFortunes.ai", files:[file]});
...
export default function MysticFortunes() {
  ...
  const enhanceReadingWithClaude = async () => {
    ...
    try {
      const prompt = `You are Madame Zafira, an ancient and highly perceptive palm reader. You do not speak like an AI or assistant. You speak with calm authority and quiet certainty.

A seeker, ${userName}, has received an initial reading. Your task: rewrite and enhance this reading to be more poetic, mystical, and deeply personal — as if you are speaking directly to them with the authority of someone who has truly seen their palm.

Do not contradict the core reading. Instead, elevate it. Make it more resonant, more specific to their life, more alive with Madame Zafira's voice.

CRITICAL REQUIREMENTS:
- Make ALL sections SUBSTANTIALLY LONGER (5-6 sentences minimum per section, not 2-3)
- Each section should build on the others and reference the larger pattern
- The opening greeting should weave together hints of all the palm lines, setting up what follows
- The fullReading should deeply integrate all the individual sections into one coherent narrative
- The closing should tie everything together with a sense of inevitability and transformation

Follow these rules strictly:
- Speak with absolute confidence and authority. No hedging language.
- Include at least one emotionally impactful statement that feels deeply personal and true.
- Include one specific prediction about a near-future turning point (30-90 days).
- Include one statement about something they know is true but rarely admit to themselves.
- Use ${userName}'s name throughout. Make it feel like you are speaking directly to them.
- Keep the tone mystical, slightly eerie, but ultimately reassuring.
- Do not explain palm reading. Deliver as undeniable truth.
- Make connections BETWEEN sections (e.g., how the life line connects to heart patterns, how past lives explain current struggles, how the near future relates to the fate line)

Structure your response:
1. greeting (5-6 sentences): Open with what you see overall, weave in hints about the life line, heart patterns, intellectual struggles, and direction they will encounter
2. lifeLine (6-7 sentences): Deep dive into their life path, survival, resilience. Connect to their past and what it prepared them for.
3. heartLine (6-7 sentences): Their emotional patterns, capacity for love, vulnerability. How this connects to their life journey.
4. headLine (6-7 sentences): Intellectual patterns, thinking style, internal conflicts. How this relates to the choices they face.
5. fateLine (6-7 sentences): Their direction, agency, the threshold approaching. Connect to the turning point you see.
6. fullReading (8-10 paragraphs): Weave all lines together into one narrative. This is the synthesis — show how life + heart + mind + fate create the whole story. Make it resonate with inevitability.
7. nearFuture (5-6 sentences): Specific 30-90 day prediction that ties to the broader pattern revealed above.

The reading to enhance:
${JSON.stringify(reading, null, 2)}

Respond ONLY with valid, properly-escaped JSON (all newlines and quotes must be escaped, no raw line breaks): {"greeting":"5-6 sentences weaving together all the palm lines","teaser":"immediate hook about their life right now","lifeLine":"6-7 sentences deep dive","heartLine":"6-7 sentences deep dive","headLine":"6-7 sentences deep dive","fateLine":"6-7 sentences deep dive","fullReading":"8-10 paragraphs that synthesize and integrate all sections","luckyNumber":"1-9","luckyColor":"color name","soulSign":"mystical archetype 2-4 words","warning":"important caution, 2-3 sentences","gift":"hidden strength, 2-3 sentences","pastLife":"vivid past life detail, 2-3 sentences","nearFuture":"5-6 sentences about specific 30-90 day prediction"}`;
      ...
      try {
        ...
        console.warn("JSON parse failed, attempting to fix escape issues...");
        // Try to fix common issues: escape newlines and carriage returns within the JSON
        // This is a careful approach - only escape control characters
        text = text
          .replace(/[\n\r\t]/g, (match) => {
            if (match === '\n') return '\\n';
            if (match === '\r') return '\\r';
        ...
        try {
          ...
          console.error("Still failed after fixing escapes:", parseErr2.message);
          console.error("Problematic text around error position:", text.substring(Math.max(0, parseErr.message.match(/position (\d+)/)? parseInt(parseErr.message.match(/position (\d+)/)[1]) - 50 : 0), parseErr.message.match(/position (\d+)/)? parseInt(parseErr.message.match(/position (\d+)/)[1]) + 50 : 100));
          throw new Error("Cannot parse Claude response: " + parseErr2.message);
        }
      }
            ...
            */}
            ...
            {!partnerPhase&&activeTab==="lines"&&(
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                ...
                {/* Madame Zafira Speaks — paywalled */}
                <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px",marginBottom:8}}>
