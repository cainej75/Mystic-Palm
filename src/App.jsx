...
const FreezeFrame = React.memo(function FreezeFrame({ palmImage, palmLandmarks, onDone, isUpload }) {
    ...
    try {
      ...
      for (let i = 0; i < data.length; i += 16) {
        ...
        if (r > 80 && g > 40 && b > 20 && r > g && r > b &&
            r - b > 15 && Math.abs(r - g) < 80) {
          skinCount++;
        }
        total++;
      }
      const ratio = skinCount / total;
      return ratio > 0.25; // At least 25% skin-tone pixels in the sample
  ...
  function validateCapture(img, iX, iY, iW, iH, W, H, lm, upload) {
    try {
      ...
      if (upload) {
        // Uploaded photo: looser position/size thresholds
        // User may not have the same framing as the silhouette guide
        const handCentred = cx > 0.15 && cx < 0.85;
        const wristLow    = wristY > 0.40;          // was 0.55
        const tipsHigh    = midTipY < 0.70;          // was 0.55
        const bigEnough   = handSpanX > 0.24 && handSpanY > 0.25; // was 0.32/0.35
        const notTooHigh  = cy < 0.88;              // was 0.80
        return (palmFacing && fingersOpen && bigEnough &&
