...
// ═══════════════════════════════════════════════════════
// MYSTIC FORTUNES — SEED ENGINE & FORTUNE GENERATOR
// ═══════════════════════════════════════════════════════
...
function generateFortune(seed, userName, dob) {
  ...
  const year = dob?.year || 1990;
  const lifeNumber = ((seed % 9) + 1);
  const MONTHS = ["January","February","March","April","May","June",
                  "July","August","September","October","November","December"];
  const birthMonth = MONTHS[(dob?.month || 1) - 1];

  const PERSONALITY_OPENERS = [
    `${name}, your soul carries the weight of someone who has lived many times before.`,
    `There is an ancient knowing in you, ${name} — a wisdom that exceeds your years.`,
    `The lines of your palm speak of a spirit born under the sign of deep transformation.`,
    `${name}, you were forged in contrast — light and shadow woven into every decision you make.`,
    `Few carry the mark you do, ${name}. The universe chose carefully when it shaped your path.`,
  ];
  const PERSONALITY_TRAITS = [
    "You feel things others miss. Crowds drain you; solitude restores you.",
    "You carry an inner fire that others are drawn to, sometimes without knowing why.",
    "You have walked through fire before. The scars are invisible but real.",
    "You are someone people trust without understanding why. That is a rare gift.",
    "You possess an intuition so sharp it has frightened you more than once.",
    "You have a gift for seeing through surfaces — people, situations, and yourself.",
  ];
  const PERSONALITY_SHADOW = [
    "But this sensitivity comes at a cost — you absorb the emotions of rooms you enter.",
    "The shadow side: you hold onto things long after they have served their purpose.",
    "Your greatest struggle is not with others — it is with the version of yourself you are afraid to become.",
    "You have protected yourself so well that sometimes even you cannot find the door.",
    "There is a tendency to give until you are hollow, then wonder why you feel empty.",
  ];
  const personality = `${seededPick(rng, PERSONALITY_OPENERS)} ${seededPick(rng, PERSONALITY_TRAITS)} ${seededPick(rng, PERSONALITY_SHADOW)}`;

  const LOVE = [
    `Your heart line curves with rare depth. You do not love lightly — and those who have been loved by you carry it forever. A connection you once dismissed as impossible is about to make itself impossible to ignore. Born in ${birthMonth}, your soul is aligned for a significant emotional encounter before the next season turns.`,
    `The heart line reveals someone who loves with totality or not at all. You have been cautious recently, and with reason. But the stars suggest the walls you built for protection have begun to keep out things you actually want. Something — or someone — is approaching that will quietly dismantle every careful defence.`,
    `Love for you has never been simple. Your line shows depth, passion, and the kind of devotion that makes others jealous. A chapter that felt closed is not fully closed. Whether you return to it or finally release it, ${birthMonth} births hold a special power over connection — expect movement.`,
  ];
  const love = seededPick(rng, LOVE);

  const CAREER = [
    `Your head line runs long and clear — a mind built for strategy and vision. The coming months carry a professional inflection point that will feel like disruption but is actually acceleration. Life number ${lifeNumber} souls thrive when they trust their instincts over consensus. The opportunity approaching will require courage, not caution.`,
    `There is a crossroads visible in your palm. Two paths. One is safe and slow. The other carries risk but also the kind of fulfilment that makes the first option feel hollow in hindsight. You already know which one calls to you. The lines say you have been delaying the choice, not because you don't know the answer — but because you do.`,
    `The fate line speaks of a professional identity you have not yet fully stepped into. There is work you are meant to do in the world that the world has not yet seen. Something shifts within the next 90 days — a conversation, an encounter, or a decision that opens a door you thought had closed.`,
  ];
  const career = seededPick(rng, CAREER);

  const MONEY = [
    `Financial energy is shifting for those born in ${birthMonth}. The cycle of scarcity you have moved through — whether material or psychological — is ending. A source of income or resource you have not yet considered will present itself. Life path ${lifeNumber} carries a particular relationship with abundance: it arrives when you stop chasing it and start trusting it.`,
    `Your palm shows an interesting pattern around material resources — periods of contraction followed by rapid expansion. You are in transition. The numbers suggest a financial decision made in the next 30 days carries unusual weight. Trust the number that feels right before the calculation, not after.`,
    `Money has always been complicated energy for you — not because you can't create it, but because you have conflicted feelings about deserving it. The universe is waiting for you to resolve that conflict. When you do, things move quickly.`,
  ];
  const money = seededPick(rng, MONEY);

  const FUTURE_EVENTS = [
    `In the next 90 days, an unexpected encounter will reframe how you see your own potential. You will recognise it by the way it makes you simultaneously terrified and certain.`,
    `Before the season turns, a door you stopped looking at will open. Go through it without overanalysing — the time for caution on this particular thing is over.`,
    `A message, a meeting, or a moment of stillness will arrive with unusual clarity. Pay attention to what you feel in the first 10 seconds — that response is the truth before the mind edits it.`,
    `Something you have been carrying for a long time is ready to be set down. The universe will offer you a moment to release it. The moment will feel small. Take it anyway.`,
    `An alliance — professional, personal, or creative — is forming that will matter more than it appears to at first. Treat it with unusual care.`,
  ];
  const futureEvent = seededPick(rng, FUTURE_EVENTS);

  const WARNINGS = [
    `Heed this: your pattern of choosing what is familiar over what is right is the one thing that can delay everything the stars have prepared for you.`,
    `The warning encoded in your palm is clear — what you tolerate, you teach. Something in your life needs to be addressed, not managed.`,
    `There is one relationship — you will know which — where the balance of giving and receiving has become dangerous. The stars ask you to see it clearly.`,
    `You have a gift that you have been treating as ordinary. The warning is this: what you do not value, others will not value either. Claim it.`,
    `Beware the version of yourself that shrinks to make others comfortable. That is not humility — that is abandonment of your own soul.`,
  ];
  const warning = seededPick(rng, WARNINGS);

  const SOUL_SIGNS = [
    "The Midnight Oracle","The Wounded Healer","The Silent Storm",
    "The Ancient Wanderer","The Hidden Flame","The Threshold Guardian",
    "The Veiled Prophet","The Unfinished Star","The Deep Water",
  ];
  const COLORS = [
    "Midnight Indigo","Blood Amber","Eclipse Silver","Ash Violet",
    "Bone White","Storm Teal","Ember Gold","Shadow Crimson","Mist Grey",
  ];

  const soulSign    = seededPick(rng, SOUL_SIGNS);
  const luckyColor  = seededPick(rng, COLORS);
  const luckyNumber = lifeNumber;

  const teaser = `The palm does not lie, ${name}. What I see encoded in your lines is both beautiful and sobering — a soul poised at the edge of its most important chapter. There is something the universe has been building toward in your life for longer than you realise. I have seen only the surface. The depths are far more revealing.`;

  const fullReading = `${personality}\n\n${love}\n\n${career}\n\nThis is not chance that brought you here, ${name}. The lines of your palm carry a specific frequency — a signature that marks you as someone standing at the beginning of something the rest of your life will be shaped by. The question is not whether the change is coming. It is whether you will step toward it, or wait for it to arrive.`;

  const greeting = `The ancient lines speak clearly for ${name} — what I see is rare.`;

  return { greeting, teaser, personality, love, career, money, futureEvent, warning,
           soulSign, luckyColor, luckyNumber, fullReading,
           lifeLine: love, heartLine: love, headLine: career, fateLine: futureEvent,
           pastLife: `Before this life, ${name}'s soul walked a path of ${seededPick(rng,["scholarship and quiet sacrifice","passion and artistic creation","leadership and eventual exile","healing and unrecognised service","adventure and necessary loss"])}. The echoes of that journey are visible in the choices you make today without fully understanding why.`,
           nearFuture: futureEvent, gift: `Your gift — the one even you underestimate — is ${seededPick(rng,["the ability to understand people before they reveal themselves","an intuition about timing that has saved you more times than you know","the capacity to transmute pain into meaning","a creative vision that others cannot see until you make it real","the rare ability to hold space for others while quietly falling apart inside"])}.` };
}

function generateCompatibility(seed1, seed2) {
  const rng = seededRandom((seed1 ^ seed2 ^ (seed1 * 7 + seed2 * 13)) >>> 0);
  const raw = Math.floor(rng() * 100);
  // Bias toward high scores (more shareable/satisfying)
  const score = Math.min(99, Math.max(42, Math.round(raw * 0.45 + 50)));

  const VERDICTS = [
    [90, "Twin Flames",      "Your energies are so aligned they amplify each other — which makes both the love and the friction more intense than anything either of you has known."],
    [80, "Kindred Spirits",  "Written in two different languages that somehow say the same thing. You complete patterns in each other that have been unfinished for longer than either of you knows."],
    ...
    [60, "Complicated Stars","This is not an easy connection. The planets that govern your birth signs pull in different directions. But difficult does not mean wrong — sometimes the path that demands the most is the one that leads furthest."],
    [0,  "Cosmic Contrast",  "You are teaching each other something. It may not always feel like love — but the universe rarely sends us people we don't need, even when we can't explain why."],
  ];
  ...
  const STRENGTHS = [
    ...
    "Your combined energy creates something neither of you could build separately.",
    "The loyalty between these two signatures is encoded deeply — this connection does not fade easily.",
    "There is an intellectual resonance here that sustains what physical attraction begins.",
  ];
  const CHALLENGES = [
    "The challenge: one of you needs space precisely when the other needs closeness.",
    "You will push each other's edges — that is not a flaw in the connection, it is the point of it.",
    "Power dynamics will need conscious attention. Both signatures carry strong will.",
    "Communication is where this connection either deepens or fractures. Choose words carefully.",
  ];
  ...
  return {
    ...
    verdict,
    explanation,
    strength: seededPick(rng, STRENGTHS),
    challenge: seededPick(rng, CHALLENGES),
  };
}
...
function generateShareText(reading, score) {
  if (score !== undefined) {
    const lines = [
      `We just found out we're ${score}% compatible 🔥 Mystic Fortune doesn't lie.`,
      `${score}% soul compatibility. I'm not okay. 😳 — Mystic Fortune`,
      `The palm reading said ${score}%. My hands confirmed it. 🙌`,
    ];
    const rng = seededRandom(score * 1337);
    return seededPick(rng, lines) + " mysticfortunes.app";
  }
  const hooks = [
    `My destiny was revealed... and I wasn't ready.`,
    `This palm reading said things about me that nobody knows.`,
    `I let an AI read my palm and now I can't stop thinking about it.`,
    `Soul Sign: "${reading?.soulSign}" 🔮 The accuracy is unsettling.`,
    `The warning it gave me? Painfully accurate. 😶`,
  ];
  const rng = seededRandom(reading?.luckyNumber * 7919 || 42);
  return seededPick(rng, hooks) + " — Mystic Fortune\nmysticfortunes.app";
}
    ...
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"#080510"}}>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"0 20px 48px",pointerEvents:"none"}}>
        <div style={{background:"rgba(8,5,16,0.88)",border:"1px solid rgba(148,90,210,0.45)",borderRadius:12,padding:"14px 22px",display:"flex",alignItems:"center",gap:14,backdropFilter:"blur(16px)"}}>
          ...
          <span style={{fontFamily:"Cinzel,serif",fontSize:12,color:"#9b5fd4",letterSpacing:2.5,textTransform:"uppercase",animation:"fadeIn 0.3s ease"}} key={status}>{status}</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"0 16px 90px",position:"relative",zIndex:2}}>
        ...
        {phase==="captured"&&palmImage&&(
          <div style={{paddingTop:36,animation:"fadeUp 0.5s ease both"}}>
            ...
            <button className="gold-btn" onClick={()=>setPhase("biometric")} style={{width:"100%",padding:"15px",fontSize:15,borderRadius:12,marginBottom:8}}>🔮 Reveal My Destiny</button>
        ...
        {phase==="loading"&&(
          <div style={{position:"fixed",inset:0,zIndex:80,background:"rgba(8,5,16,0.96)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24}}>
            ...
            <div style={{fontFamily:"Cinzel,serif",fontSize:15,color:"#f5d060",letterSpacing:3,textTransform:"uppercase",animation:"auraPulse 2s ease-in-out infinite",textShadow:"0 0 16px rgba(245,208,96,0.8)"}}>Reading your ancient lines…</div>
          </div>
        )}
        ...
        {phase==="result"&&reading&&(
          <div style={{paddingTop:28,paddingBottom:80,animation:"fadeUp 0.6s ease both"}}>
            ...
            <div style={{textAlign:"center",margin:"10px 0 14px"}}>
              ...
              {userName&&birthDate&&(()=>{const s=generateSeed(userName,birthDate);const f=generateFortune(s,userName,birthDate);return(<div style={{fontFamily:"Cinzel,serif",fontSize:9,color:C.muted,letterSpacing:2,textTransform:"uppercase",marginTop:6}}>Life Path {(s%9)+1} · {f.soulSign}</div>);})()}
            </div>
              ...
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                ...
                {!paid&&!subscribed&&(
                  <div style={{background:"linear-gradient(135deg,#100616,#0c0812)",border:`1px solid ${C.gold}38`,borderRadius:14,padding:"18px 16px",marginBottom:10,textAlign:"center",animation:"orbGlow 4s ease-in-out infinite"}}>
                    ...
                    <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",color:C.cream,fontSize:14,lineHeight:1.85,margin:"0 0 12px",opacity:0.9}}>"I have shown you only a glimpse of your future... The full truth is far more revealing... If you wish to uncover what truly awaits you, you must allow me to complete the reading."</p>
                    <p style={{fontFamily:"Crimson Text,serif",fontStyle:"italic",color:C.muted,fontSize:12,lineHeight:1.6,margin:"0 0 10px"}}>Full palm analysis · Past life echo · Complete mystical narrative · Fate line reading</p>
