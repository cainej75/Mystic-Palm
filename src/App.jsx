...
const FullScreenShareCard = React.memo(function FullScreenShareCard({reading, userName, birthDate, onClose, todayFortune, onSaved}) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      inAppDownload(url, 'mystic-fortune.png', (u) => { setLongPressUrl(u); setSaving(false); return; });
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"linear-gradient(160deg,rgba(8,3,20,0.98) 0%,rgba(40,8,25,0.98) 50%,rgba(8,3,20,0.98) 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:1000,overflowY:"auto"}}>
      ...
      {longPressUrl&&(
        <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,0.95)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{fontFamily:"Cinzel,serif",fontSize:14,color:"#c9a84c",letterSpacing:1,marginBottom:16,textAlign:"center"}}>Hold down on the image and tap <strong>Save Image</strong></div>
          <img src={longPressUrl} alt="Your share card" style={{maxWidth:"100%",maxHeight:"70vh",borderRadius:12,border:"2px solid #c9a84c"}}/>
          <button onClick={()=>setLongPressUrl(null)} style={{marginTop:20,background:"none",border:"1px solid #c9a84c",color:"#c9a84c",borderRadius:8,padding:"10px 24px",fontFamily:"Cinzel,serif",fontSize:12,cursor:"pointer",letterSpacing:1}}>← Back</button>
        </div>
      )}
    ...
    try {
      ...
      script.onload = async () => {
        try {
          ...
          const url = canvas.toDataURL('image/png');
          inAppDownload(url, 'cosmic-connection.png', (u) => { setLongPressUrl(u); setSaving(false); });
...
function PartnerCompatibilityResult({ reading, userName, partnerName, nameDisplayOrder, onBack, setActiveTab, pdfPaid, pdfGenerating, pdfNarrative, onPDFPayment, birthDate, partnerBirthDate, soulmateAnswers, soulmateJustCurious, initialTab }) {
  ...
  const shareConnection = async () => {
    ...
    try {
      ...
      const url = canvas.toDataURL("image/png");
      inAppDownload(url, "cosmic-connection.png", (u) => { setLongPressUrl(u); setCapturing(false); });
      if (!isInAppBrowser()) { setCapturing(false); setShared(true); }
  ...
  return (
    <div ref={pageRef} style={{position:"fixed",inset:0,zIndex:9999,background:"#080510",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      {longPressUrl&&(
        <div style={{position:"fixed",inset:0,zIndex:10001,background:"rgba(0,0,0,0.95)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{fontFamily:"Cinzel,serif",fontSize:14,color:"#c9a84c",letterSpacing:1,marginBottom:16,textAlign:"center"}}>Hold down on the image and tap <strong>Save Image</strong></div>
          <img src={longPressUrl} alt="Your share card" style={{maxWidth:"100%",maxHeight:"70vh",borderRadius:12,border:"2px solid #c9a84c"}}/>
          <button onClick={()=>{setLongPressUrl(null);setShared(true);}} style={{marginTop:20,background:"none",border:"1px solid #c9a84c",color:"#c9a84c",borderRadius:8,padding:"10px 24px",fontFamily:"Cinzel,serif",fontSize:12,cursor:"pointer",letterSpacing:1}}>← Back</button>
        </div>
      )}
      {/* Leave Warning Modal */}
      {showLeaveWarning&&(
...
Respond ONLY with valid JSON, no preamble, no markdown:
{
  ...
  return(
    <>
    {/* Global PDF long-press overlay for in-app browsers */}
    {pdfLongPressUrl&&(
      <div style={{position:"fixed",inset:0,zIndex:99999,background:"rgba(0,0,0,0.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{fontSize:40,marginBottom:16}}>📜</div>
        <div style={{fontFamily:"Cinzel,serif",fontSize:16,color:"#c9a84c",letterSpacing:1,marginBottom:12,textAlign:"center",fontWeight:700}}>Your PDF is Ready</div>
        <div style={{fontFamily:"Crimson Text,serif",fontStyle:"italic",fontSize:16,color:"#e8d5b8",marginBottom:28,textAlign:"center",lineHeight:1.7,maxWidth:320}}>
          Tap <strong style={{color:"#c9a84c"}}>Open PDF</strong> below. When it opens, use the share icon or long-press to save it to your device.
        </div>
        <button
          onClick={()=>{
            // Navigate current tab to blob — most reliable approach in in-app browsers
            window.location.href = pdfLongPressUrl.url;
            // Mark as saved after short delay
            setTimeout(()=>{
              if(pdfLongPressUrl.type==='full') setFullPDFSaved(true);
              if(pdfLongPressUrl.type==='partner') setDeepPDFDownloaded(true);
              setPdfLongPressUrl(null);
            }, 1500);
          }}
          style={{background:"linear-gradient(135deg,#c9a84c,#9d7d2e)",color:"#080510",fontFamily:"Cinzel,serif",fontSize:15,fontWeight:700,letterSpacing:1.5,padding:"16px 40px",borderRadius:12,border:"none",cursor:"pointer",marginBottom:16,width:"100%",maxWidth:300}}
        >
          📜 Open PDF
        </button>
        <button onClick={()=>{
          ...
          setPdfLongPressUrl(null);
        }} style={{background:"none",border:"1px solid rgba(201,168,76,0.4)",color:"rgba(201,168,76,0.7)",borderRadius:8,padding:"10px 24px",fontFamily:"Cinzel,serif",fontSize:12,cursor:"pointer",letterSpacing:1}}>
          Cancel
        </button>
      </div>
    )}
