...
// ── OpenInBrowserOverlay — reusable "open in browser" flow ───────────────────
// Mirrors the logic from CameraBlockedPage / InAppBrowserGate
function OpenInBrowserOverlay({ onDismiss, title, subtitle }) {
  const [deepLinkTried, setDeepLinkTried] = React.useState(false);
  const [showManual, setShowManual] = React.useState(false);
  const ua = navigator.userAgent || '';
  const isAndroid = /Android/i.test(ua);
  const platform = /Instagram/i.test(ua) ? 'Instagram' : /FBAN|FBAV/i.test(ua) ? 'Facebook' : /TikTok/i.test(ua) ? 'TikTok' : 'this app';
  ...
  const tryOpenInBrowser = () => {
    ...
    if (!isAndroid) {
      try { window.location.href = appUrl.replace(/^https?:\/\//, 'safari-https://'); } catch(e) {}
      setTimeout(() => { try { window.open(appUrl, '_system'); } catch(e) {} }, 200);
    } else {
      try {
        const u = new URL(appUrl);
        window.location.href = `intent://${u.hostname}${u.pathname}${u.search}#Intent;scheme=https;package=com.android.chrome;action=android.intent.action.VIEW;end`;
      } catch(e) {}
      setTimeout(() => { try { window.open(appUrl, '_system'); } catch(e) {} }, 300);
      setTimeout(() => { try { window.location.href = appUrl; } catch(e) {} }, 600);
      setTimeout(() => { try { window.open(appUrl, '_blank'); } catch(e) {} }, 900);
    }
  ...
  const gold = '#c9a84c';
  return (
    <div style={{position:'fixed',inset:0,zIndex:99999,background:'#080510',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'28px 24px',overflowY:'auto'}}>
      <div style={{position:'absolute',top:'-30%',left:'-30%',width:'70%',height:'70%',background:'radial-gradient(circle,rgba(201,168,76,0.1),transparent)',borderRadius:'50%',pointerEvents:'none'}}/>
      <div style={{position:'absolute',bottom:'-20%',right:'-20%',width:'60%',height:'60%',background:'radial-gradient(circle,rgba(120,60,180,0.08),transparent)',borderRadius:'50%',pointerEvents:'none'}}/>

      <div style={{position:'relative',zIndex:2,width:'100%',maxWidth:420,display:'flex',flexDirection:'column',alignItems:'center',gap:20}}>

        <h2 style={{fontFamily:"Cinzel,serif",fontSize:'clamp(20px,5.5vw,28px)',fontWeight:700,color:gold,margin:0,textAlign:'center',letterSpacing:0.5,textShadow:'0 0 24px rgba(201,168,76,0.4)',lineHeight:1.25}}>
          {showManual ? <>That didn't work,<br/>do this instead</> : (title || <>Downloading isn't possible<br/>in {platform}</>)}
        </h2>

        {!showManual && (
          <p style={{fontFamily:"Cinzel,serif",fontSize:'clamp(11px,3vw,13px)',color:'#ffb347',margin:0,letterSpacing:1,textTransform:'uppercase',fontWeight:600,textAlign:'center'}}>
            ⚠️ {platform} blocks file downloads
          </p>
        )}

        {subtitle && !deepLinkTried && (
          <p style={{fontFamily:"Crimson Text,serif",fontStyle:'italic',fontSize:'clamp(15px,4vw,18px)',color:'#e8d5b8',margin:0,textAlign:'center',lineHeight:1.6}}>{subtitle}</p>
        )}

        {!deepLinkTried && (
          <button onClick={tryOpenInBrowser} style={{width:'100%',padding:'18px',background:'linear-gradient(135deg,#1a0a2e,#2e1250,#1a0a2e)',border:'2px solid #c9a84c',borderRadius:14,fontFamily:"Cinzel,serif",fontSize:'clamp(13px,3.8vw,16px)',fontWeight:700,letterSpacing:1,color:'#e8d5b8',cursor:'pointer',boxShadow:'0 4px 28px rgba(201,168,76,0.25)',animation:'buttonPulse 2s ease-in-out infinite'}}>
            ✦ Open in Browser
          </button>
        )}

        {deepLinkTried && !showManual && (
          <p style={{fontFamily:"IM Fell English,serif",fontStyle:'italic',fontSize:'clamp(15px,4vw,18px)',color:'#c084fc',margin:0,textAlign:'center',textShadow:'0 0 18px rgba(192,132,252,0.5)'}}>
            Opening your browser…
          </p>
        )}

        {showManual && (
          <div style={{background:'rgba(201,168,76,0.08)',border:'1px solid rgba(201,168,76,0.3)',borderRadius:16,padding:'28px',width:'100%',boxSizing:'border-box'}}>
            <p style={{margin:'0 0 18px',fontFamily:"Cinzel,serif",fontSize:'clamp(14px,4vw,18px)',fontWeight:700,color:gold,textAlign:'center'}}>Open your browser manually</p>
            {[['1.', <>Tap the three dots <strong style={{color:'#ffe08a'}}>(⋮)</strong> or menu at the top of the screen</>],
              ['2.', <>Tap <strong style={{color:'#ffe08a'}}>"Open in browser"</strong></>]].map(([n,text])=>(
              <div key={n} style={{display:'flex',alignItems:'flex-start',gap:14,marginBottom:14}}>
                <span style={{fontFamily:"Cinzel,serif",fontSize:'clamp(16px,4.5vw,20px)',color:gold,fontWeight:700,flexShrink:0,marginTop:2}}>{n}</span>
                <p style={{margin:0,fontFamily:"Crimson Text,serif",fontSize:'clamp(17px,5vw,21px)',color:'#d4c5a9',lineHeight:1.6}}>{text}</p>
              </div>
            ))}
            <p style={{margin:'12px 0 0',fontFamily:"Cinzel,serif",fontSize:'clamp(12px,3.5vw,14px)',color:'#c084fc',textAlign:'center',fontWeight:600,letterSpacing:0.5,textShadow:'0 0 18px rgba(192,132,252,0.7)'}}>
              Once in your browser you can download your file.
            </p>
          </div>
        )}

        <button onClick={onDismiss} style={{background:'none',border:'none',color:'rgba(201,168,76,0.5)',fontFamily:"Cinzel,serif",fontSize:12,cursor:'pointer',letterSpacing:1,marginTop:4}}>
          {showManual ? 'Close' : 'Cancel'}
        </button>
      </div>
    </div>
  );
...
function isInAppBrowser() {
  const ua = navigator.userAgent || "";
  return /FBAN|FBAV|Instagram|LinkedInApp|Twitter|Snapchat|TikTok|musical_ly/i.test(ua);
}

// ── InAppDownloadGate — blocks Facebook/Instagram users before they start ─────
function InAppDownloadGate({ platform, onDismiss }) {
  const [deepLinkTried, setDeepLinkTried] = React.useState(false);
  const [showManual, setShowManual] = React.useState(false);
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:99999,background:'#080510',display:'flex',flexDirection:'column',alignItems:'center',overflowY:'auto',boxSizing:'border-box'}}>
      <div style={{position:'absolute',top:'-30%',left:'-30%',width:'70%',height:'70%',background:'radial-gradient(circle,rgba(201,168,76,0.12),transparent)',borderRadius:'50%',pointerEvents:'none'}}/>
      <div style={{position:'absolute',bottom:'-20%',right:'-20%',width:'60%',height:'60%',background:'radial-gradient(circle,rgba(176,64,90,0.1),transparent)',borderRadius:'50%',pointerEvents:'none'}}/>

      <div style={{position:'relative',width:'100%',maxWidth:440,flexShrink:0,paddingTop:24}}>
        <img src="/crystal-ball.webp" alt="Crystal ball" width="700" height="443"
          style={{width:'100%',height:'auto',display:'block',objectFit:'cover',borderRadius:'0 0 4px 4px'}}/>
        <div style={{position:'absolute',bottom:0,left:0,right:0,height:'30%',background:'linear-gradient(to bottom,transparent,#080510)',pointerEvents:'none'}}/>
      </div>

      <div style={{position:'relative',zIndex:2,width:'100%',maxWidth:440,padding:'20px 24px 36px',boxSizing:'border-box',display:'flex',flexDirection:'column',alignItems:'center',gap:16}}>
        <h1 style={{fontFamily:"Cinzel,serif",fontSize:'clamp(22px,6vw,30px)',fontWeight:700,color:gold,margin:0,lineHeight:1.2,letterSpacing:0.5,textAlign:'center',textShadow:'0 0 24px rgba(201,168,76,0.4)'}}>
          {showManual ? <>That didn't work,<br/>do this instead</> : <>{platform} blocks<br/>downloads & saving</>}
        </h1>

        {!showManual && (
          <p style={{fontFamily:"Crimson Text,serif",fontStyle:'italic',fontSize:'clamp(16px,4.5vw,19px)',color:'#e8d5b8',margin:0,textAlign:'center',lineHeight:1.65}}>
            To save your share card and PDF, Madame Zafira needs you in your real browser. It only takes a moment.
          </p>
        )}

        <p style={{fontFamily:"Cinzel,serif",fontSize:'clamp(11px,3vw,13px)',color:'#ffb347',margin:0,letterSpacing:1,textTransform:'uppercase',fontWeight:600,textAlign:'center'}}>
          ⚠️ {platform} is blocking file downloads
        </p>
        ...
        {showManual && (
          <div style={{background:'rgba(201,168,76,0.08)',border:'1px solid rgba(201,168,76,0.3)',borderRadius:16,padding:'28px',width:'100%',boxSizing:'border-box'}}>
            <p style={{margin:'0 0 18px',fontFamily:"Cinzel,serif",fontSize:'clamp(14px,4vw,18px)',fontWeight:700,color:gold,textAlign:'center'}}>Open your browser manually</p>
            {[['1.',<>Tap the three dots <strong style={{color:'#ffe08a'}}>(⋮)</strong> at the top of the screen</>],
              ['2.',<>Tap <strong style={{color:'#ffe08a'}}>"Open in browser"</strong></>]].map(([n,text])=>(
              <div key={n} style={{display:'flex',alignItems:'flex-start',gap:14,marginBottom:14}}>
                <span style={{fontFamily:"Cinzel,serif",fontSize:'clamp(16px,4.5vw,20px)',color:gold,fontWeight:700,flexShrink:0,marginTop:2}}>{n}</span>
                <p style={{margin:0,fontFamily:"Crimson Text,serif",fontSize:'clamp(17px,5vw,21px)',color:'#d4c5a9',lineHeight:1.6}}>{text}</p>
            ...
            <p style={{margin:'12px 0 0',fontFamily:"Cinzel,serif",fontSize:'clamp(12px,3.5vw,14px)',color:'#c084fc',textAlign:'center',fontWeight:600,letterSpacing:0.5,textShadow:'0 0 18px rgba(192,132,252,0.7)'}}>
              Takes 5 seconds. Unlocks everything.
            </p>
          </div>
        )}

        <button onClick={onDismiss} style={{background:'none',border:'none',color:'rgba(201,168,76,0.45)',fontFamily:"Cinzel,serif",fontSize:12,cursor:'pointer',letterSpacing:1,marginTop:4}}>
          ← Back
        </button>
      </div>

      <div style={{width:'100%',display:'flex',flexDirection:'column',alignItems:'center',gap:8,marginTop:'auto',paddingTop:20,paddingBottom:16,borderTop:'1px solid rgba(201,168,76,0.15)'}}>
        <p style={{fontFamily:"Cinzel,serif",fontSize:'clamp(13px,3.5vw,16px)',color:'rgba(201,168,76,0.85)',margin:0,letterSpacing:'2px'}}>🔮 mysticfortunes.ai</p>
      </div>
      <style>{`@keyframes buttonPulse{0%,100%{box-shadow:0 4px 28px rgba(201,168,76,0.25),0 0 20px rgba(201,168,76,0.1)}50%{box-shadow:0 8px 40px rgba(201,168,76,0.5),0 0 40px rgba(201,168,76,0.3)}}`}</style>
    </div>
  );
}

// ── Download helper — shows long-press UI inside in-app browsers ─────────────
function inAppDownload(dataUrl, filename, onShowLongPress) {
  if (isInAppBrowser()) {
    // In-app browsers block programmatic downloads entirely — show image for long-press
    if (onShowLongPress) onShowLongPress(dataUrl);
    return;
  }
  try {
...
const FullScreenShareCard = React.memo(function FullScreenShareCard({reading, userName, birthDate, onClose, todayFortune, onSaved}) {
  ...
  const handleSave = useCallback(async () => {
    ...
    // In-app browsers: skip html2canvas, just show the card for screenshotting
    if (inApp) {
    ...
    try {
      ...
      setSaving(false);
      // Fallback: show screenshot overlay
      setShowScreenshot(true);
    }
  }, [saving, saved, inApp]);

  // Screenshot overlay — shown in in-app browsers or on html2canvas failure
  const cardContent = (
    <>
      ...
      <div style={{textAlign:"center"}}>
        <span style={{fontFamily:"Cinzel,serif",fontSize:10,color:"#c9a84c",letterSpacing:1}}>🔮 MysticFortunes.ai</span>
      </div>
    </>
  );

  if (showOpenBrowser) return <OpenInBrowserOverlay onDismiss={()=>setShowOpenBrowser(false)} subtitle="Open mysticfortunes.ai in your browser to save your share card."/>;

  if (showScreenshot) {
    return (
      <div style={{position:"fixed",inset:0,zIndex:2000,background:"#080510",display:"flex",flexDirection:"column",alignItems:"center",overflowY:"auto",padding:"20px 20px 40px"}}>
        <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:"#c9a84c",letterSpacing:1,textAlign:"center",marginBottom:16,padding:"10px 16px",background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:10,width:"100%",maxWidth:380}}>
          📸 Screenshot this screen to save your card
        </div>
        <div style={{maxWidth:330,width:"100%",background:"#0d0920",border:"3px solid #c9a84c",borderRadius:20,padding:"14px 10px",marginBottom:24}}>
          {cardContent}
        </div>
        <button onClick={()=>{setShowScreenshot(false);setSaved(true);if(onSaved)onSaved();}}
          style={{background:"#1a6b4a",border:"1px solid #2ecc2e",color:"white",borderRadius:10,padding:"12px 30px",fontSize:12,fontFamily:"Cinzel,serif",fontWeight:600,cursor:"pointer",letterSpacing:1,textTransform:"uppercase",marginBottom:10,width:"100%",maxWidth:330}}>
          ✓ Done — I've Taken a Screenshot
        </button>
        <button onClick={()=>setShowOpenBrowser(true)}
          style={{background:"none",border:"1px solid rgba(201,168,76,0.4)",color:"rgba(201,168,76,0.7)",borderRadius:8,padding:"10px 24px",fontFamily:"Cinzel,serif",fontSize:12,cursor:"pointer",letterSpacing:1,marginBottom:8,width:"100%",maxWidth:330}}>
          ✦ Open in Browser Instead
        </button>
        <button onClick={()=>setShowScreenshot(false)}
          style={{background:"none",border:"none",color:"rgba(201,168,76,0.5)",fontFamily:"Cinzel,serif",fontSize:11,cursor:"pointer",letterSpacing:1}}>
          ← Back
        </button>
      </div>
    );
...
const PartnerFullScreenShareCard = React.memo(function PartnerFullScreenShareCard({partnerReading, userName, partnerName, nameDisplayOrder, onClose}) {
  ...
  const handleSave = useCallback(async () => {
    if (saving || saved) return;
    if (inApp) { setShowScreenshot(true); return; }
    setSaving(true);
    try {
      if (!window.html2canvas) {
      ...
      const canvas = await window.html2canvas(shareCardRef.current, { scale:1, backgroundColor:'#080510', logging:false, useCORS:true, allowTaint:true, imageTimeout:0, windowHeight:shareCardRef.current.scrollHeight+100, windowWidth:400 });
      const url = canvas.toDataURL('image/png');
      const arr=url.split(','); const mime=arr[0].match(/:(.*?);/)[1]; const bstr=atob(arr[1]); let n=bstr.length; const u8=new Uint8Array(n); while(n--) u8[n]=bstr.charCodeAt(n);
      const blob=new Blob([u8],{type:mime}); const blobUrl=URL.createObjectURL(blob);
      const a=document.createElement('a'); a.href=blobUrl; a.download='cosmic-connection.png'; a.style.display='none'; document.body.appendChild(a); a.click(); document.body.removeChild(a);
      setTimeout(()=>URL.revokeObjectURL(blobUrl),5000);
      setSaving(false); setSaved(true);
    } catch(err) { console.error('Save failed:',err); setSaving(false); setShowScreenshot(true); }
  }, [saving, saved, inApp]);
  ...
  const cardInner = (
    <>
      ...
      <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:"#c9a84c",letterSpacing:1,textAlign:"center",fontWeight:700}}>🔮 mysticfortunes.ai</div>
    </>
  );
  ...
  if (showScreenshot) {
    return (
      <div style={{position:"fixed",inset:0,zIndex:2000,background:"#080510",display:"flex",flexDirection:"column",alignItems:"center",overflowY:"auto",padding:"20px 20px 40px"}}>
        ...
        <div style={{maxWidth:280,width:"100%",background:"#080510",border:"2px solid #c9a84c",borderRadius:14,padding:"16px",marginBottom:24}}>
          {cardInner}
        </div>
        <button onClick={()=>{setShowScreenshot(false);setSaved(true);}}
          style={{background:"#1a6b4a",border:"1px solid #2ecc2e",color:"white",borderRadius:10,padding:"12px 30px",fontSize:12,fontFamily:"Cinzel,serif",fontWeight:600,cursor:"pointer",letterSpacing:1,textTransform:"uppercase",marginBottom:10,width:"100%",maxWidth:280}}>
          ✓ Done — I've Taken a Screenshot
        </button>
        <button onClick={()=>setShowOpenBrowser(true)}
          style={{background:"none",border:"1px solid rgba(201,168,76,0.4)",color:"rgba(201,168,76,0.7)",borderRadius:8,padding:"10px 24px",fontFamily:"Cinzel,serif",fontSize:12,cursor:"pointer",letterSpacing:1,marginBottom:8,width:"100%",maxWidth:280}}>
          ✦ Open in Browser Instead
        </button>
        <button onClick={()=>setShowScreenshot(false)} style={{background:"none",border:"none",color:"rgba(201,168,76,0.5)",fontFamily:"Cinzel,serif",fontSize:11,cursor:"pointer",letterSpacing:1}}>← Back</button>
      </div>
    );
  }
...
const FullscreenCamera = React.memo(function FullscreenCamera({ canvasRef, onCapture, onCancel, onUpload }) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const W = oc.offsetWidth || window.innerWidth || v.videoWidth || 390;
      const H = oc.offsetHeight || window.innerHeight || v.videoHeight || 844;
...
const FreezeFrame = React.memo(function FreezeFrame({ palmImage, palmLandmarks, onDone, isUpload }) {
  const canvasRef = useRef(null);
  ...
  useEffect(() => {
    ...
    img.onload = async () => {
      ...
      const valid = validateCapture(img, iX, iY, iW, iH, W, H, palmLandmarks, isUpload);
      if (!valid) {
  ...
  function skinToneFallback(img, iX, iY, iW, iH) {
    // Basic pixel check — sample centre of image for skin-tone pixels
    try {
      const c = document.createElement('canvas');
      const sampleW = 200, sampleH = 300;
      c.width = sampleW; c.height = sampleH;
      const cx = c.getContext('2d');
      // Sample the centre third of the image where the palm should be
      cx.drawImage(img,
        img.width * 0.25, img.height * 0.15, img.width * 0.5, img.height * 0.7,
        0, 0, sampleW, sampleH
      );
      ...
      for (let i = 0; i < data.length; i += 16) {
        ...
        // Broad skin tone range covering light to dark skin
        if (r > 60 && g > 30 && b > 15 && r > b &&
            r - b > 10 && Math.abs(r - g) < 100) {
          skinCount++;
        }
        total++;
      }
      const ratio = skinCount / total;
      return ratio > 0.15; // At least 15% skin-tone pixels in the sample
    } catch(e) { return false; }
  }

  function validateCapture(img, iX, iY, iW, iH, W, H, lm, upload) {
    try {
      // No landmarks — for uploads try pixel fallback, for live scan fail
      if (!lm || lm.length < 21) {
        if (upload) return skinToneFallback(img, iX, iY, iW, iH);
        return false;
      }
      ...
      // Hand positioning — looser thresholds for uploads
      const wristY    = lm[0].y;
      const midTipY   = lm[12].y;
      ...
      if (upload) {
        // Uploaded photo: allow either hand, very generous thresholds
        // Palm facing: accept either orientation (left or right hand)
        const palmFacingEither = Math.abs(cross) > 0.01; // just needs some orientation
        const handCentred = cx > 0.05 && cx < 0.95;
        const wristLow    = wristY > 0.25;
        const tipsHigh    = midTipY < 0.85;
        const bigEnough   = handSpanX > 0.15 && handSpanY > 0.15;
        const notTooHigh  = cy < 0.95;
        return (palmFacingEither && fingersOpen && bigEnough &&
                handCentred && wristLow && tipsHigh && notTooHigh);
      } else {
        // Live camera: original strict thresholds — don't touch
        const handCentred = cx > 0.25 && cx < 0.75;
...
function PartnerCompatibilityResult({ reading, userName, partnerName, nameDisplayOrder, onBack, setActiveTab, pdfPaid, pdfGenerating, pdfNarrative, onPDFPayment, birthDate, partnerBirthDate, soulmateAnswers, soulmateJustCurious, initialTab }) {
  ...
  const [activeResultTab, setActiveResultTab] = useState(initialTab || "soulmate");
  ...
  const [longPressUrl, setLongPressUrl] = useState(null);

  const shareConnection = async () => {
    if (capturing || shared) return;
    if (isInAppBrowser()) { setLongPressUrl('screenshot'); return; }
    setCapturing(true);
    try {
    ...
    } catch(e) { console.error(e); setCapturing(false); setLongPressUrl('screenshot'); }
  };
  ...
  return (
    <div ref={pageRef} style={{position:"fixed",inset:0,zIndex:9999,background:"#080510",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      {showPDFBrowser&&<OpenInBrowserOverlay onDismiss={()=>setShowPDFBrowser(false)} title="Can't download PDF here" subtitle="Open mysticfortunes.ai in your browser, then go through the soulmate flow to download your PDF."/>}
      {/* Long-press save overlay for in-app browsers */}
      {longPressUrl==='openBrowser'&&(
        <OpenInBrowserOverlay onDismiss={()=>setLongPressUrl(null)} subtitle="Open mysticfortunes.ai in your browser to download your share card."/>
      )}
      {longPressUrl==='screenshot'&&(
        <div style={{position:"fixed",inset:0,zIndex:10001,background:"rgba(0,0,0,0.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:"#c9a84c",letterSpacing:1,textAlign:"center",marginBottom:16,padding:"10px 16px",background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:10,maxWidth:380,width:"100%"}}>
            📸 Screenshot this screen to save your card
          </div>
          <div style={{background:"linear-gradient(160deg,#0c0818 0%,#100c1a 50%,#0a0614 100%)",border:"3px solid rgba(201,168,76,0.85)",borderRadius:18,padding:"28px 22px 24px",marginBottom:20,position:"relative",overflow:"hidden",maxWidth:380,width:"100%"}}>
            ...
            <div style={{textAlign:"center",marginBottom:12,fontFamily:"Cinzel,serif",fontSize:18,color:rose,letterSpacing:1,fontWeight:600}}>{name1} &amp; {name2}</div>
            ...
            <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:14,color:cream,margin:"0 0 16px",lineHeight:1.75,textAlign:"center"}}>"{insight}"</p>
          ...
          <button onClick={()=>{setLongPressUrl(null);setShared(true);}} style={{background:"#1a6b4a",border:"1px solid #2ecc2e",color:"white",borderRadius:10,padding:"12px 30px",fontFamily:"Cinzel,serif",fontSize:12,cursor:"pointer",letterSpacing:1,textTransform:"uppercase",marginBottom:10,width:"100%",maxWidth:380}}>
            ✓ Done — I've Taken a Screenshot
          </button>
          <button onClick={()=>setLongPressUrl('openBrowser')} style={{background:"none",border:"1px solid rgba(201,168,76,0.4)",color:"rgba(201,168,76,0.7)",borderRadius:8,padding:"10px 24px",fontFamily:"Cinzel,serif",fontSize:12,cursor:"pointer",letterSpacing:1,marginBottom:8,width:"100%",maxWidth:380}}>
            ✦ Open in Browser Instead
          </button>
          <button onClick={()=>setLongPressUrl(null)} style={{background:"none",border:"none",color:"rgba(201,168,76,0.5)",fontFamily:"Cinzel,serif",fontSize:11,cursor:"pointer",letterSpacing:1}}>← Back</button>
        </div>
      )}
      {/* Leave Warning Modal */}
      {showLeaveWarning&&(
        ...
        {activeResultTab==="deep"&&(
          <div style={{animation:"fadeIn 0.3s ease both"}}>
            ...
            {!pdfPaid ? (
              ...
              <div style={{position:"fixed",inset:0,zIndex:9999,background:"#060412",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                {(()=>{
                  const SoulmateLoadingInner = () => {
                    const [cosmosPhase,setCosmosPhase]=React.useState(false);
                    const msgs=["Reading the lines of your connection…","The cosmos aligns your souls…","Consulting the ancient stars…","Madame Zafira sees the bond…","Your deeper connection is being revealed…"];
                    const [msgIdx,setMsgIdx]=React.useState(0);
                    React.useEffect(()=>{const t=setTimeout(()=>setCosmosPhase(true),3500);return()=>clearTimeout(t);},[]);
                    React.useEffect(()=>{const t=setInterval(()=>setMsgIdx(i=>(i+1)%msgs.length),2000);return()=>clearInterval(t);},[]);
                    return !cosmosPhase?(
                      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:28,animation:"fadeIn 0.6s ease both"}}>
                        <div style={{position:"relative",width:160,height:160}}>
                          <div style={{position:"absolute",inset:0,borderRadius:"50%",background:"radial-gradient(circle at 40% 35%,#8b2fc9,#3b0a6e 60%,#0d0520)",boxShadow:"0 0 80px rgba(139,47,201,0.7),inset 0 0 50px rgba(192,144,255,0.2)",animation:"orbGlow 3s ease-in-out infinite"}}/>
                          <div style={{position:"absolute",top:"18%",left:"22%",width:"35%",height:"22%",borderRadius:"50%",background:"rgba(255,255,255,0.15)",filter:"blur(4px)",transform:"rotate(-20deg)"}}/>
                          <div style={{position:"absolute",inset:0,borderRadius:"50%",border:"2px solid rgba(192,144,255,0.5)",animation:"spin 8s linear infinite"}}/>
                          <div style={{position:"absolute",inset:8,borderRadius:"50%",border:"1px solid rgba(201,168,76,0.35)",animation:"spin 5s linear infinite reverse"}}/>
                          <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:36}}>💗</div>
                        </div>
                        <div style={{textAlign:"center",maxWidth:300}}>
                          <div style={{fontFamily:"Cinzel,serif",fontSize:15,color:"#c090ff",letterSpacing:2.5,textTransform:"uppercase",animation:"auraPulse 2s ease-in-out infinite",marginBottom:10}}>{msgs[msgIdx]}</div>
                          <div style={{display:"flex",gap:8,justifyContent:"center"}}>{[0,1,2].map(i=>(<div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#c090ff",opacity:0.4,animation:`auraPulse 1.2s ease-in-out ${i*0.4}s infinite`}}/>))}</div>
                        </div>
                      </div>
                    ):(
                      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:28,animation:"fadeIn 0.8s ease both"}}>
                        <div style={{position:"relative",width:"min(320px,85vw)",height:"min(220px,60vw)",borderRadius:18,overflow:"hidden",boxShadow:"0 0 80px rgba(139,47,201,0.6)"}}>
                          <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,#020008 0%,#0d0635 35%,#1a0530 65%,#000005 100%)"}}/>
                          {[...Array(50)].map((_,i)=>{const s=0.5+Math.random()*2.5;return(<div key={i} style={{position:"absolute",left:`${Math.random()*100}%`,top:`${Math.random()*100}%`,width:s,height:s,borderRadius:"50%",background:i%4===0?"#c090ff":"white",opacity:0.1+Math.random()*0.8,animation:`auraPulse ${1.5+Math.random()*3}s ease-in-out ${Math.random()*2}s infinite`}}/>);})}
                          <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
                            <div style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:"clamp(14px,3.8vw,18px)",color:"rgba(230,210,255,0.9)",textAlign:"center",lineHeight:1.7,textShadow:"0 0 20px rgba(192,144,255,0.6)"}}>
                              "Two souls, two palms, one story the stars have been composing all along…"
                            </div>
                          </div>
                        </div>
                        <div style={{textAlign:"center"}}>
                          <div style={{fontFamily:"Cinzel,serif",fontSize:15,color:"#c090ff",letterSpacing:2.5,textTransform:"uppercase",animation:"auraPulse 2s ease-in-out infinite",marginBottom:10}}>Madame Zafira is writing your reading…</div>
                          <div style={{fontFamily:"Crimson Text,serif",fontStyle:"italic",fontSize:16,color:"rgba(192,144,255,0.6)",marginBottom:10}}>The cosmos cannot be rushed.</div>
                          <div style={{display:"flex",gap:8,justifyContent:"center"}}>{[0,1,2].map(i=>(<div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#c090ff",opacity:0.4,animation:`auraPulse 1.2s ease-in-out ${i*0.4}s infinite`}}/>))}</div>
                        </div>
                      </div>
                    );
                  };
                  return <SoulmateLoadingInner/>;
                })()}
              </div>
            ) : pdfNarrative ? (
              <>
                ...
                <button
                  onClick={async()=>{
                    if(isInAppBrowser()){ setShowPDFBrowser(true); return; }
                    try {
...
export default function MysticFortunes() {
  ...
  const [palmFromUpload,setPalmFromUpload]=useState(false);
  const [partnerFromUpload,setPartnerFromUpload]=useState(false);
  ...
  const [pdfLongPressUrl,setPdfLongPressUrl]=useState(null);
  const [showFullPDFBrowser,setShowFullPDFBrowser]=useState(false);
  ...
  useEffect(()=>{
    ...
    if(phase==="result"){setSpeaking(false);return;}
    setSpeaking(false);
  },[phase]);
...
{"greeting":"${userName}, [2-3 sentences weaving palm impression with celestial signature]","lifeLine":"[3-4 sentences — life path energy, stamina, reinvention, Chinese zodiac influence if relevant]","heartLine":"[3-4 sentences — emotional patterns, love nature, moon sign resonance]","headLine":"[3-4 sentences — mental style, intuition, ruling planet influence]","fateLine":"[3-4 sentences — soul-level destiny direction, life path number significance, what is being built over years not weeks]","fullReading":"[4 paragraphs separated by \n\n: 1.Overall cosmic portrait. 2.The central challenge written in both palm and stars. 3.What is approaching in their life. 4.The unique gift only they carry.]","nearFuture":"[2-3 sentences — specific 30-90 day celestial and palm prediction, distinct from fateLine]"}`;
  ...
  const generateSoulmateNarrative = async (answers, justCurious, uName, uDob, pName, pDob) => {
    if (soulmateNarrativeGenerating) return;
    setSoulmateNarrativeGenerating(true);
    // If no answers available, go straight to fallback
    if (!answers) {
      console.warn("No soulmate answers — using fallback narrative");
      const getSignFB = (dob) => { if (!dob) return "Unknown"; try { return getZodiac(dob)?.sign || "Unknown"; } catch { return "Unknown"; } };
      const ZODIAC_META_FB = { Aries:{element:"Fire",modality:"Cardinal"},Taurus:{element:"Earth",modality:"Fixed"},Gemini:{element:"Air",modality:"Mutable"},Cancer:{element:"Water",modality:"Cardinal"},Leo:{element:"Fire",modality:"Fixed"},Virgo:{element:"Earth",modality:"Mutable"},Libra:{element:"Air",modality:"Cardinal"},Scorpio:{element:"Water",modality:"Fixed"},Sagittarius:{element:"Fire",modality:"Mutable"},Capricorn:{element:"Earth",modality:"Cardinal"},Aquarius:{element:"Air",modality:"Fixed"},Pisces:{element:"Water",modality:"Mutable"} };
      const uS = getSignFB(uDob); const pS = getSignFB(pDob);
      setSoulmateNarrative(generateFallbackSoulmateNarrative({q1:"chance",q2:"",q3:"their warmth",q4:""}, justCurious, uName, uS, pName, pS, ZODIAC_META_FB[uS]||{}, ZODIAC_META_FB[pS]||{}));
      setSoulmateNarrativeGenerating(false);
      return;
    }
    const getSign = (dob) => {
      if (!dob) return "Unknown";
      try { return getZodiac(dob)?.sign || "Unknown"; } catch { return "Unknown"; }
    };
    ...
    const connectionType = justCurious ? "connection" : "soulmate connection";
...
${sameElement ? `Both share the ${userMeta.element||"same"} element.` : `${userMeta.element||"their"} meets ${partMeta.element||"their"}.`}
...
Respond ONLY with valid JSON, no preamble, no markdown:
{
  ...
  "celestialCompatibility": "3-4 sentences — their ${userSign} and ${partSign} interplay. Cover elements (${userMeta.element||"their"} + ${partMeta.element||"their"}), what each sign needs, where they complement, where they clash. Be specific and accurate — this is real astrology.",
  ...
  return(
    <>
    {/* In-app browser gate — shown when Facebook/Instagram user taps a journey */}
    {showInAppGate&&(
      <InAppDownloadGate
        platform={/Instagram/i.test(navigator.userAgent)?'Instagram':/FBAN|FBAV/i.test(navigator.userAgent)?'Facebook':/TikTok|musical_ly/i.test(navigator.userAgent)?'TikTok':'this app'}
        onDismiss={()=>{setShowInAppGate(false);setShowNavDrawer(true);}}
      />
    )}
    {/* Full Revelation PDF — in-app browser overlay */}
    {showFullPDFBrowser&&(
      <OpenInBrowserOverlay
        onDismiss={()=>setShowFullPDFBrowser(false)}
        title="Can't download PDF here"
        subtitle="Open mysticfortunes.ai in your browser, complete your reading, then download your PDF."
      />
    )}
    {showNatalScreen&&(
      <NatalInfoScreen
    ...
    {partnerPhase==="result"&&partnerReadingResult&&(
      <PartnerCompatibilityResult
        ...
        initialTab={partnerPDFPaid ? "deep" : "soulmate"}
      />
    )}
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"0 16px 24px",position:"relative",zIndex:2}}>
        ...
        <NavDrawer
          ...
          onStartReading={(featureId)=>{
            // Gate Facebook/Instagram users before they invest time
            if(isInAppBrowser()){
              setPendingJourney(featureId);
        ...
        {phase==="freeze"&&palmImage&&(
          <FreezeFrame
            ...
            isUpload={palmFromUpload}
            onDone={(valid)=>{
              setPalmFromUpload(false);
        ...
        {partnerPhase==="freeze"&&partnerPalmImage&&(
          <FreezeFrame
            ...
            isUpload={partnerFromUpload}
            onDone={(valid)=>{
              setPartnerFromUpload(false);
            ...
            return(
              <div style={{position:"fixed",inset:0,zIndex:80,background:"#060412",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                {!cosmosPhase?(
                  ...
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:28,animation:"fadeIn 0.8s ease both"}}>
                    <div style={{position:"relative",width:"min(320px,88vw)",height:"min(220px,60vw)",borderRadius:18,overflow:"hidden",boxShadow:"0 0 80px rgba(100,60,180,0.6)"}}>
                      <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,#020008 0%,#0d0635 35%,#050218 65%,#000005 100%)"}}/>
                      {[...Array(50)].map((_,i)=>{const s=0.5+Math.random()*2.5;return(<div key={i} style={{position:"absolute",left:`${Math.random()*100}%`,top:`${Math.random()*100}%`,width:s,height:s,borderRadius:"50%",background:"white",opacity:0.1+Math.random()*0.8,animation:`auraPulse ${1.5+Math.random()*3}s ease-in-out ${Math.random()*2}s infinite`}}/>);})}
                      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <div style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:"clamp(15px,4vw,19px)",color:"rgba(230,210,255,0.85)",textAlign:"center",padding:"0 24px",lineHeight:1.7,textShadow:"0 0 20px rgba(180,140,255,0.5)"}}>
                          "What the heavens wrote on the night you were born, the palm confirmed…"
                        </div>
                      </div>
                    </div>
                    <div style={{textAlign:"center"}}>
                      <div style={{fontFamily:"Cinzel,serif",fontSize:15,color:"#c9a84c",letterSpacing:2.5,textTransform:"uppercase",animation:"auraPulse 2s ease-in-out infinite",marginBottom:10}}>Consulting the cosmos…</div>
                      <div style={{display:"flex",gap:8,justifyContent:"center"}}>{[0,1,2].map(i=>(<div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#c9a84c",opacity:0.3,animation:`auraPulse 1.2s ease-in-out ${i*0.4}s infinite`}}/>))}</div>
                    </div>
                  </div>
                )}
              </div>
                ...
                {(fullPaid||subscribed)&&(
                  ...
                  <button
                    onClick={async()=>{
                      ...
                      if(isInAppBrowser()){ setShowFullPDFBrowser(true); return; }
                      setSavingFullPDF(true);
                      try{
                        const{generateFullRevelationPDF}=await lazyImportPDFGenerators();
                        await generateFullRevelationPDF(reading,userName,birthDate);
                        setFullPDFSaved(true);
                      }catch(e){console.error(e);}
