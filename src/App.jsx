...
function ShareCard({reading, onOpenFullscreen}) {
  return (
...
function FreezeFrame({ palmImage, palmLandmarks, onDone }) {
  ...
  function validateCapture(img, iX, iY, iW, iH, W, H, lm) {
    try {
      ...
      tc.drawImage(img, 0, 0, img.width, img.height,
        -iX*(RES/iW), -iY*(RES/iH),
        img.width*(RES/iW), img.height*(RES/iH)
      );
      ...
      for (let i = 0; i < d.length; i += 4) {
        ...
        if (r > 90 && g > 45 && b > 20 &&
            r > g && r > b && (r - g) > 12 &&
            r < 245 && bright > 60 && bright < 220) {
          skinCount++;
        }
      }
      const skinRatio = skinCount / (RES * RES);
      if (skinRatio < 0.18) return false;
      if (lm && lm.length >= 21) {
        const spanX = Math.abs(lm[4].x - lm[20].x);
        const spanY = Math.abs(lm[0].y - lm[12].y);
        const fingersUp = lm[12].y < lm[0].y;
        const centred   = lm[9].x > 0.12 && lm[9].x < 0.88;
        if (!fingersUp || !centred || spanX < 0.18 || spanY < 0.25) return false;
      }
      return true;
...
Respond ONLY with valid JSON, no markdown: {"greeting":"dramatic 1-sentence max 18 words","teaser":"2-3 sentence teaser with hook","lifeLine":"2 vivid sentences","heartLine":"2 vivid sentences","headLine":"2 vivid sentences","fateLine":"2 vivid sentences","fullReading":"4 paragraphs with \\n\\n","luckyNumber":"1-9","luckyColor":"evocative colour","soulSign":"mystical archetype 2-4 words","warning":"one ominous sentence","gift":"one sentence","pastLife":"one past life hint","nearFuture":"one 90-day prediction"}`;
  ...
  return(
    <>
    {(phase==="freeze" && palmImage) || (partnerPhase==="freeze" && partnerPalmImage)?(
      <FreezeFrame
        palmImage={partnerPhase==="freeze"?partnerPalmImage:palmImage}
        palmLandmarks={palmLandmarks}
        onDone={(valid)=>{
          if(partnerPhase==="freeze"){
      ...
      <BiometricScan
        ...
        onComplete={()=>{
          if(partnerPhase==="biometric"){
            // Generate partner compatibility reading
    ...
    <div style={{minHeight:"100vh",background:C.bg,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"0 16px 90px",position:"relative",zIndex:2}}>
        ...
        {/* ══ FULLSCREEN CAMERA ══ */}
        {(phase==="capture"&&cameraOn) || (partnerPhase==="capture"&&partnerCameraOn)?(
          <FullscreenCamera
            key={partnerPhase==="capture"?"partner":"regular"}
            canvasRef={partnerPhase==="capture"?partnerCanvasRef:canvasRef}
            onCapture={(img,lm)=>{
              if(partnerPhase==="capture"){
                setPartnerPalmImage(img);
                setPartnerPhase("captured");
              }else{
                setPalmImage(img);
        ...
        {phase==="captured"&&palmImage&&(
          <div style={{paddingTop:36,animation:"fadeUp 0.5s ease both"}}>
            ...
            <div style={{textAlign:"center",marginBottom:16}}>
              <h2 style={{fontFamily:"Cinzel,serif",fontSize:24,color:C.gold,margin:"0 0 4px"}}>I See Your Hand…</h2>
            </div>
            ...
            <div style={{borderRadius:12,overflow:"hidden",margin:"14px 0",border:`1px solid ${C.border}`,position:"relative"}}>
              ...
              <div style={{position:"absolute",bottom:8,left:"50%",transform:"translateX(-50%)",fontFamily:"Cinzel,serif",fontSize:14,color:C.gold,letterSpacing:2.5,textTransform:"uppercase",whiteSpace:"nowrap"}}>✦ Palm Captured ✦</div>
            </div>
            <button className="gold-btn" onClick={()=>setPhase("biometric")} style={{width:"100%",padding:"15px",fontSize:20,borderRadius:12,marginBottom:8}}>🔮 Reveal My Destiny</button>
            <button className="ghost-btn" onClick={()=>{setPalmImage(null);setPhase("capture");}} style={{width:"100%",padding:"9px",fontSize:16,borderRadius:9}}>↺ Retake Photo</button>
          </div>
        )}

        {/* ══ PARTNER CAPTURED ══ */}
        {partnerPhase==="captured"&&partnerPalmImage&&(
          <div style={{paddingTop:36,animation:"fadeUp 0.5s ease both"}}>
            <div style={{textAlign:"center",marginBottom:16}}>
              <h2 style={{fontFamily:"Cinzel,serif",fontSize:24,color:C.gold,margin:"0 0 4px"}}>I See Their Hand…</h2>
              <p style={{fontFamily:"Crimson Text,serif",fontSize:14,color:C.cream,margin:0}}>Your cosmic connection awaits</p>
            </div>
            <Seraphina speaking={false} phase="reading" mood="gift" videoRef={videoRef2} muted={muted} setMuted={setMuted}/>
            <div style={{height:16}}/>
            <div style={{borderRadius:12,overflow:"hidden",margin:"14px 0",border:`1px solid ${C.rose}`,position:"relative"}}>
              <img src={partnerPalmImage} alt="Partner's palm" style={{width:"100%",maxHeight:220,objectFit:"cover",display:"block",filter:"sepia(25%) contrast(1.08)"}}/>
              <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 52%,rgba(8,5,16,0.88) 100%)"}}/>
              <div style={{position:"absolute",bottom:8,left:"50%",transform:"translateX(-50%)",fontFamily:"Cinzel,serif",fontSize:14,color:C.rose,letterSpacing:2.5,textTransform:"uppercase",whiteSpace:"nowrap"}}>✦ Palm Captured ✦</div>
            </div>
            <button className="gold-btn" onClick={()=>setPartnerPhase("biometric")} style={{width:"100%",padding:"15px",fontSize:20,borderRadius:12,marginBottom:8}}>💕 Reveal Our Connection</button>
            <button className="ghost-btn" onClick={()=>{setPartnerPalmImage(null);setPartnerPhase("capture");}} style={{width:"100%",padding:"9px",fontSize:16,borderRadius:9}}>↺ Retake Photo</button>
          </div>
        )}
            ...
            */}
                ...
                {showPartnerDetailsPage&&!paid?(
                  ...
                  <div style={{animation:"fadeIn 0.3s"}}>
                    ...
                    <div style={{textAlign:"center",marginBottom:24}}>
                      ...
                      <h2 style={{fontFamily:"Cinzel,serif",fontSize:22,color:C.gold,margin:"0 0 12px",letterSpacing:1}}>Partner Compatibility Reading</h2>
                ...
                {(paid||subscribed)&&(
                  <div style={{animation:"fadeUp 0.6s ease both"}}>
                    <div style={{background:C.surface,border:`1px solid ${C.gold}2a`,borderRadius:14,padding:"16px",marginBottom:10}}>
                      ...
                      <div style={{fontFamily:"Cinzel,serif",fontSize:15,color:C.goldDim,letterSpacing:3,textTransform:"uppercase",marginBottom:12,textAlign:"center",color:C.gold,fontSize:17}}>✦ The Full Reading ✦</div>
                      {reading.pastLife&&(
                      ...
                      {[["💗 Heart Line",reading.heartLine],["🧠 Head Line",reading.headLine],["✋ Life Line",reading.lifeLine]].map(([title,text])=>(
                      ...
                      <Divider/>
                      <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.goldDim,letterSpacing:3,textTransform:"uppercase",marginBottom:10,textAlign:"center",color:C.gold}}>✦ Madame La Voyante Speaks ✦</div>
            ...
            {activeTab==="lines"&&(
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                ...
                <div style={{background:"linear-gradient(135deg,rgba(25,12,45,0.8),rgba(45,15,28,0.8))",border:`1px solid ${C.gold}2a`,borderRadius:12,padding:"13px",marginBottom:8}}>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.gold,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6,cursor:"pointer",userSelect:"none"}} onClick={()=>{devPaidTapRef.current=(devPaidTapRef.current||0)+1;clearTimeout(devPaidTimerRef.current);if(devPaidTapRef.current>=3){devPaidTapRef.current=0;setPaid(true);setActiveTab("lines");}else{devPaidTimerRef.current=setTimeout(()=>{devPaidTapRef.current=0;},1200);}}}>🌟 The Next 30 Days</div>
                  {(paid||subscribed)?
            ...
            {activeTab==="extras"&&(
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                ...
                <div style={{marginBottom:12}}>
                  <ShareCard reading={reading} onOpenFullscreen={()=>setShowFullscreenShareCard(true)}/>
                </div>
            ...
            <Divider/>
            {activeTab==="extras"&&(
              <button onClick={()=>{setPhase("intro");setReading(null);setPalmImage(null);setPaid(false);setSubscribed(false);setSpeaking(false);}}
            ...
            {/* Footer Links */}
            <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:20,paddingTop:16,borderTop:`1px solid ${C.border}`,flexWrap:"wrap"}}>
          ...
          {partnerPhase==="result"&&partnerReadingResult&&(
            <div style={{animation:"fadeIn 0.6s ease both"}}>
              <div style={{textAlign:"center",marginBottom:24}}>
                ...
                <h2 style={{fontFamily:"Cinzel,serif",fontSize:26,color:C.gold,margin:"0 0 8px",letterSpacing:1}}>Your Cosmic Connection</h2>
                <p style={{fontFamily:"Crimson Text,serif",fontSize:15,color:C.cream}}>The universe has spoken about your alignment</p>
              </div>

              {/* Compatibility Score */}
              <div style={{background:C.surface,border:`2px solid ${C.gold}`,borderRadius:14,padding:"24px",marginBottom:20,textAlign:"center"}}>
                <div style={{fontFamily:"Cinzel,serif",fontSize:14,color:C.goldDim,letterSpacing:1.5,textTransform:"uppercase",marginBottom:12}}>Harmony Score</div>
                <div style={{fontFamily:"Cinzel,serif",fontSize:56,color:"#ff6b8a",fontWeight:700,marginBottom:8}}>{partnerReadingResult.score}%</div>
                <p style={{fontFamily:"Crimson Text,serif",fontSize:16,color:C.cream,margin:0,lineHeight:1.8}}>{partnerReadingResult.insight}</p>
              </div>

              {/* Alignment Description */}
              <div style={{background:`rgba(42, 138, 122, 0.15)`,border:`1px solid ${C.teal}44`,borderRadius:12,padding:"16px",marginBottom:20}}>
                <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.teal,letterSpacing:1.5,textTransform:"uppercase",marginBottom:8}}>🌙 Your Alignment</div>
                <p style={{fontFamily:"Crimson Text,serif",fontSize:15,color:C.cream,margin:0,lineHeight:1.8}}>
                  The cosmic forces align beautifully between you. Your destinies are woven together by the universe, creating a bond that transcends the physical realm. This connection holds both challenges and profound beauty.
                </p>
              </div>

              {/* Share Card Preview */}
              <div style={{background:C.bg,border:`2px solid ${C.gold}`,borderRadius:14,padding:"16px",marginBottom:16,textAlign:"center",position:"relative",overflow:"hidden",maxWidth:280,margin:"0 auto 16px"}}>
                <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:`linear-gradient(90deg,transparent,${C.gold},transparent)`}}/>
                
                <div style={{fontFamily:"Cinzel,serif",fontSize:20,color:C.gold,marginBottom:8,letterSpacing:2}}>✦ Mystic Fortunes ✦</div>
                
                <div style={{fontFamily:"Cinzel,serif",fontSize:16,color:C.gold,marginBottom:3}}>Soulmate Connection</div>
                <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:"#ff6b8a",letterSpacing:1,marginBottom:12}}>
                  {nameDisplayOrder==="userFirst"?`${userName||"Your Name"} & ${partnerName||"Partner's Name"}`:`${partnerName||"Partner's Name"} & ${userName||"Your Name"}`}
                </div>
                ...
                <div style={{marginBottom:12}}>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:10,color:C.gold,letterSpacing:1,textTransform:"uppercase",marginBottom:3}}>Date</div>
                  <div style={{fontFamily:"Crimson Text,serif",fontSize:12,color:C.cream}}>{new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                </div>
                
                <div style={{marginBottom:12}}>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:32,color:"#ff6b8a",fontWeight:700}}>{partnerReadingResult.score}%</div>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:9,color:"#ff9db8",letterSpacing:1,textTransform:"uppercase"}}>Harmony Score</div>
                </div>
                
                <div style={{height:1,background:`linear-gradient(90deg,transparent,${C.gold}44,transparent)`,marginBottom:12}}/>
                
                <p style={{fontFamily:"Crimson Text,serif",fontStyle:"italic",fontSize:12,color:C.cream,lineHeight:1.7,margin:"0 0 12px"}}>{partnerReadingResult.insight}</p>
              ...
              {/* Share Button */}
              <button className="gold-btn" onClick={()=>setShowPartnerShareCard(true)} style={{width:"100%",padding:"14px",fontSize:16,borderRadius:11,marginBottom:12}}>
                ✨ Share Your Connection
              </button>

              {/* Back Button */}
              <button className="ghost-btn" onClick={()=>{setPartnerPhase(null);setPartnerPalmImage(null);setPartnerReadingResult(null);}} style={{width:"100%",padding:"12px",fontSize:14,borderRadius:11}}>
                ← Return to Reading
              </button>
            </div>
          )}
