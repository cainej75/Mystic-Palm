...
// ── In-app browser safe download helper ─────────────────────────────────────
// Facebook/Instagram/TikTok browsers block a.click() on data URLs.
// Strategy: try blob + click, fall back to opening image in new tab with save instructions.
function inAppDownload(dataUrl, filename) {
  try {
    // Convert data URL to blob for better in-app browser compatibility
    ...
    const w = window.open();
    if (w) {
      w.document.write(`<html><body style="margin:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;"><p style="color:white;font-family:sans-serif;text-align:center;padding:20px;">Long-press the image and tap <strong>Save Image</strong></p><img src="${dataUrl}" style="max-width:100%;"/></body></html>`);
      w.document.close();
    }
  }
}
...
const FullScreenShareCard = React.memo(function FullScreenShareCard({reading, userName, birthDate, onClose, todayFortune, onSaved}) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      inAppDownload(url, 'mystic-fortune.png');
    ...
    try {
      ...
      script.onload = async () => {
        try {
          ...
          const url = canvas.toDataURL('image/png');
          inAppDownload(url, 'cosmic-connection.png');
