...
function DetailsPage({ onSubmit, onBack }) {
  ...
  return (
    <div style={{paddingTop:40,paddingBottom:60,animation:"fadeUp 0.6s ease both"}}>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:4,paddingTop:14,paddingBottom:16,borderTop:`1px solid ${C.border}`,flexWrap:"wrap"}}>
        <Link to="/privacy-policy?from=app" target="_blank" rel="noopener noreferrer" className="footer-link">Privacy Policy</Link>
        <span style={{color:C.border}}>•</span>
        <Link to="/terms-and-conditions?from=app" target="_blank" rel="noopener noreferrer" className="footer-link">Terms & Conditions</Link>
      </div>
    </div>
  );
}
...
function PartnerCompatibilityResult({ reading, userName, partnerName, nameDisplayOrder, onBack, setActiveTab }) {
  ...
  useEffect(() => {
    window.history.pushState({partnerResult: true}, '');
    const onPop = (e) => {
      window.history.pushState({partnerResult: true}, '');
...
{/* Your Alignment section — temporarily hidden, data intact */}
        ...
        {/* Footer Links */}
        <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:4,paddingTop:14,paddingBottom:16,borderTop:"1px solid #2e1f40",flexWrap:"wrap"}}>
          <Link to="/privacy-policy?from=app" target="_blank" rel="noopener noreferrer" className="footer-link">Privacy Policy</Link>
...
export default function MysticFortunes() {
  ...
  // Intercept phone back button when Full Revelation is paid and active
  useEffect(() => {
    if (!fullPaid || subscribed || activeTab !== 'lines') return;
    window.history.pushState({fullReveal: true}, '');
    const onPop = () => {
      window.history.pushState({fullReveal: true}, '');
      setShowFullLeaveWarning(true);
      setPendingFullLeave(() => () => { window.history.go(-2); });
    };
    window.addEventListener('popstate', onPop);
    return () => window.removeEventListener('popstate', onPop);
  }, [fullPaid, subscribed, activeTab]);
  ...
  return(
    ...
    <div style={{minHeight:"100vh",background:C.bg,position:"relative",overflow:"auto"}}>
      <style>{`
        ...
        .footer-link{color:#e8c76b !important;font-family:Cinzel,serif;font-size:11px;letter-spacing:1px;text-decoration:none !important;transition:color 0.3s;}
        .footer-link:hover{color:#b0405a !important;}
        .footer-link:visited{color:#e8c76b !important;}
        .footer-link:active{color:#e8c76b !important;}
