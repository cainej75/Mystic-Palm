      ...
      if (sessionId.startsWith('free_')) {
        ...
        if (productType === 'full_revelation') {
          ...
          try {
            ...
            const savedBirthDate = sessionStorage.getItem('pendingBirthDate');
            const savedReading = sessionStorage.getItem('pendingReading');
            const restoredName = savedUserName || 'You';
            const restoredDob = savedBirthDate ? JSON.parse(savedBirthDate) : {day:1,month:1,year:1990};
            setUserName(restoredName);
            setBirthDate(restoredDob);
            if (savedReading) {
              setReading(JSON.parse(savedReading));
            } else {
              const seed = generateSeed(restoredName, restoredDob);
              setReading(generateFortune(seed, restoredName, restoredDob));
            }
            sessionStorage.removeItem('pendingUserName');
            sessionStorage.removeItem('pendingBirthDate');
            sessionStorage.removeItem('pendingReading');
          } catch(e) { console.warn('sessionStorage restore failed:', e); }
          setPartnerPaid(true);
          setPhase('result');
        ...
        if (data.valid) {
          ...
          if (data.productType === 'full_revelation') {
            ...
            try {
              ...
              setUserName(restoredName);
              setBirthDate(restoredDob);
              // Restore or regenerate reading so phase=result can render
              if (savedReading) {
                setReading(JSON.parse(savedReading));
              } else {
                const seed = generateSeed(restoredName, restoredDob);
...
Respond ONLY with valid JSON, no markdown: {"greeting":"dramatic 1-sentence max 18 words","teaser":"2-3 sentence teaser with hook","lifeLine":"2 vivid sentences","heartLine":"2 vivid sentences","headLine":"2 vivid sentences","fateLine":"2 vivid sentences","fullReading":"4 paragraphs with \\n\\n","luckyNumber":"1-9","luckyColor":"evocative colour","soulSign":"mystical archetype 2-4 words","warning":"one ominous sentence","gift":"one sentence","pastLife":"one past life hint","nearFuture":"one 90-day prediction"}`;
  ...
  const handlePartnerPayment = () => {
    // Save state needed to restore the app after Stripe page reload
    try {
      if (userName) sessionStorage.setItem('pendingUserName', userName);
      if (birthDate) sessionStorage.setItem('pendingBirthDate', JSON.stringify(birthDate));
      if (reading) sessionStorage.setItem('pendingReading', JSON.stringify(reading));
      // palmImage is too large for sessionStorage — skip it
                  ...
                  <div style={{animation:"fadeIn 0.3s"}}>
                    ...
                    <div style={{background:"linear-gradient(135deg,#100616,#0c0812)",border:`1px solid ${C.gold}38`,borderRadius:14,padding:"18px 16px",textAlign:"center",animation:"orbGlow 4s ease-in-out infinite"}}>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center",justifyContent:"center",margin:"12px 0 16px"}}>
                        <span style={{fontFamily:"Cinzel,serif",fontSize:16,color:"#c090ff",textDecoration:"line-through"}}>$5.99</span>
                        <span style={{fontFamily:"Cinzel,serif",fontSize:36,color:C.gold,fontWeight:700}}>$3.99</span>
                      </div>
                ...
                {!fullPaid&&!subscribed&&(
                  <div style={{background:"linear-gradient(135deg,#100616,#0c0812)",border:`1px solid ${C.gold}38`,borderRadius:14,padding:"18px 16px",marginBottom:10,textAlign:"center",animation:"orbGlow 4s ease-in-out infinite"}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center",justifyContent:"center",margin:"10px 0"}}>
                      ...
                      <span style={{fontFamily:"Cinzel,serif",fontSize:28,color:C.gold,fontWeight:700}}>$4.99</span>
                    ...
                    <button className="gold-btn" onClick={()=>handleFullPayment()} style={{width:"100%",padding:"12px",fontSize:16,borderRadius:11,marginBottom:6}}>🔮 Unlock Full Reading — $4.99</button>
                  </div>
                )}
              </div>
            )}
