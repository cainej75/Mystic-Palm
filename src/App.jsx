...
export default function MysticFortunes() {
  ...
  // Handle partner loading → result transition
  useEffect(()=>{
    if(partnerPhase==="loading") {
      const t = setTimeout(()=>{
        ...
        setPaid(true);
      }, 3500);  // Increased to 3.5s to match typical API response time
      return ()=>clearTimeout(t);
    }
  },[partnerPhase,userName,birthDate,partnerName,partnerBirthDate]);

  // Validation function used by both user and partner
  const validatePalmImage = (lm) => {
    try {
      // MUST have landmarks or reject immediately
      if (!lm || lm.length < 21) return false;
      
      const wrist = lm[0], indexMCP = lm[5], pinkyMCP = lm[17], midFinger = lm[12];

      // Confidence check: wrist must be clearly defined (y > 0.3 = lower on screen)
      if (wrist.z < 0.3) return false;  // Low confidence wrist detection
      
      // Landmarks must not be bunched up (sign of false detection)
      const spanX = Math.abs(lm[4].x - lm[20].x);
      const spanY = Math.abs(lm[0].y - lm[12].y);
      if (spanX < 0.1 || spanY < 0.1) return false;  // Hand too small/bunched

      // Left hand check
      const thumbLeft = lm[4].x < lm[20].x;
      if (!thumbLeft) return false;

      // Palm facing: cross product (strong check)
      const v1x = indexMCP.x - wrist.x, v1y = indexMCP.y - wrist.y;
      const v2x = pinkyMCP.x - wrist.x, v2y = pinkyMCP.y - wrist.y;
      const cross = v1x * v2y - v1y * v2x;
      if (cross <= 0.01) return false;  // Palm must be clearly facing

      // Fingers must be open and above wrist
      const fingersOpen = [8,12,16,20].every(tip => lm[tip].y < lm[tip-2].y);
      if (!fingersOpen) return false;
      
      // Finger tips must be clearly above wrist
      const topFingerY = Math.min(lm[8].y, lm[12].y, lm[16].y, lm[20].y);
      if (topFingerY > wrist.y) return false;  // Wrist must be below all fingers
      ...
      const wristLow     = wristY > 0.55;
      const tipsHigh     = midTipY < 0.55;
      const bigEnough    = spanX > 0.32 && spanY > 0.35;
      const notTooHigh   = cy < 0.80;
    ...
    <div style={{minHeight:"100vh",background:C.bg,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"0 16px 90px",position:"relative",zIndex:2}}>
        ...
        {partnerPhase==="loading"&&(
          <div style={{position:"fixed",inset:0,zIndex:80,background:"rgba(8,5,16,0.96)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24}}>
            <div style={{width:60,height:60,borderRadius:"50%",border:"3px solid rgba(201,168,76,0.2)",borderTop:`3px solid ${C.gold}`,animation:"spin 1s linear infinite"}}/>
            <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:"#f5d060",letterSpacing:2,textTransform:"uppercase",animation:"auraPulse 2s ease-in-out infinite",textShadow:"0 0 12px rgba(245,208,96,0.8)"}}>Reading their ancient lines…</div>
          </div>
        )}
