...
import { generatePartnerCompatibilityPDF } from "./generatePartnerCompatibilityPDF";
import { generateFullRevelationPDF } from "./generateFullRevelationPDF";
...
function FullScreenShareCard({reading, userName, birthDate, onClose, todayFortune}) {
  ...
  return(
    ...
    <div style={{minHeight:"100vh",background:C.bg,position:"relative",overflow:"auto"}}>
            ...
            {!partnerPhase&&activeTab==="lines"&&(
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                ...
                <div style={{textAlign:"center",marginBottom:20}}>
                  ...
                  <div style={{fontFamily:"Cinzel,serif",fontSize:17,color:C.gold,letterSpacing:2,marginBottom:4}}>{new Date().toLocaleDateString('en-AU',{day:'numeric',month:'long',year:'numeric'})}</div>
      ...
      {showFullscreenShareCard && reading && (
        <FullScreenShareCard reading={reading} userName={userName} birthDate={birthDate} onClose={()=>setShowFullscreenShareCard(false)} todayFortune={(()=>{const n=userName||"Seeker";const dayOfYear=Math.floor((Date.now()-new Date(new Date().getFullYear(),0,0))/86400000);const dailySeed=(generateSeed(userName,birthDate)^(dayOfYear*2654435761))>>>0;const dRng=seededRandom(dailySeed);const DF=[`The universe conspires in your favour today, ${n} — but only if you take the first step. Something you have been waiting for permission to begin needs no permission but your own. Trust that the timing is perfect, even when it feels uncertain.`,`Today carries an unusual momentum, ${n}. The action you have been postponing is precisely the one the universe is waiting for you to take. Your hesitation has served its purpose — now let it go.`,`A door that has been closed for some time is looser on its hinges today, ${n}. A gentle push is all it requires. What you thought was locked away may be more accessible than you believe.`,`The stars have cleared a path for you today, ${n}. The obstacle you were preparing to navigate has moved. Walk forward without waiting for confirmation. The universe has already given it.`,`Today is not a day for planning, ${n}. It is a day for doing the thing the plan has been delaying. Overthinking is the last refuge of fear — move instead.`,`Something small you do today, ${n}, will have a disproportionately large effect. The universe is amplifying your signal. Pay attention to the ripples you create.`,`The hesitation you have been feeling is not intuition warning you away, ${n}. It is habit. Today, act before the habit speaks. Your intuition lives deeper than fear.`,`Today brings a narrow window, ${n}. Move when you feel the pull, not after you have reasoned yourself out of it. Windows close quickly once you stop moving toward them.`,`The energy today favours beginnings, ${n}. Whatever you start carries further than what you start later. The momentum you build now will carry you through doubt.`,`A conversation you have been avoiding will be easier today than tomorrow, ${n}. The longer you wait, the heavier it becomes. The lightness you seek lives on the other side of speaking.`];return DF[Math.floor(dRng()*DF.length)];})()}/>
      )}
