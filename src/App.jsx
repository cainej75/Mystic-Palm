  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        hands.setOptions({ maxNumHands:1, modelComplexity:0, minDetectionConfidence:0.3, minTrackingConfidence:0.2 });
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (lm) {
        ...
        // Left hand check — works across mirrored and non-mirrored cameras
        const label = lastResultsRef.current?.multiHandedness?.[0]?.label;
        // Thumb position is the most reliable cross-device check:
        // Left palm facing camera: thumb tip (lm[4]) has HIGHER x than pinky tip (lm[20])
        // This holds true whether or not the camera stream is mirrored
        const thumbRight = lm[4].x > lm[20].x;
        // Also accept MediaPipe label as backup — mirrored cameras: Right=left, non-mirrored: Left=left
        const labelSaysLeft = label === "Right" || label === "Left";
        const isLeftHand = thumbRight || labelSaysLeft;

        // Palm facing: cross product wrist→indexMCP × wrist→pinkyMCP
        const v1x = indexMCP.x - wrist.x, v1y = indexMCP.y - wrist.y;
        const v2x = pinkyMCP.x - wrist.x, v2y = pinkyMCP.y - wrist.y;
        const cross = v1x * v2y - v1y * v2x;
        // Palm facing check: cross product of wrist->indexMCP x wrist->pinkyMCP
        // Mirrored camera (most phones): palm up left hand gives cross > 0
        // Non-mirrored camera (some Samsung): palm up left hand gives cross < 0
        // Palm DOWN on either: cross is near 0 or very small magnitude
        // So: accept if |cross| > 0.04 (hand is flat-facing, not edge-on)
        // This rejects palm-down (edge-on = small cross) while accepting both mirror types
        const palmFacing = Math.abs(cross) > 0.04;
        ...
        const wristLow     = wristY > 0.48;          // wrist near bottom of silhouette
        const tipsHigh     = midTipY < 0.62;         // fingertips in upper half
        const bigEnough    = handSpanX > 0.28 && handSpanY > 0.30;  // hand fills silhouette
...
function FreezeFrame({ palmImage, palmLandmarks, onDone }) {
  ...
  function validateCapture(img, iX, iY, iW, iH, W, H, lm) {
    try {
      ...
      const bigEnough    = handSpanX > 0.28 && handSpanY > 0.30;
      const notTooHigh   = cy < 0.80;
    ...
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"#080510",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      <div style={{maxWidth:480,margin:"0 auto",padding:"20px 16px 80px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:20}}>
          ...
          <h2 style={{fontFamily:"Cinzel,serif",fontSize:26,fontWeight:700,color:gold,margin:"0 0 8px",letterSpacing:2}}>
            Your Cosmic Connection
          </h2>
          <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:15,color:muted,margin:0}}>
            The universe has unveiled your alignment
          </p>
        </div>

        <div style={{background:"rgba(201,168,76,0.06)",border:"1px solid rgba(201,168,76,0.4)",borderRadius:16,padding:"24px 20px",marginBottom:20}}>
          <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:gold,letterSpacing:4,textTransform:"uppercase",textAlign:"center",marginBottom:14}}>
            &#x2726; Harmony Score &#x2726;
          </div>
          <div style={{textAlign:"center",marginBottom:14}}>
            <span style={{fontFamily:"Cinzel,serif",fontSize:64,fontWeight:700,color:rose,lineHeight:1}}>
              {score}%
            </span>
          </div>
          <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:15,color:cream,margin:0,lineHeight:1.8,textAlign:"center"}}>
            {insight}
          </p>
        </div>
        ...
        <div style={{marginBottom:24}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
            ...
            <span style={{fontFamily:"Cinzel,serif",fontSize:11,color:gold,letterSpacing:3,textTransform:"uppercase"}}>Your Alignment</span>
            <div style={{flex:1,height:1,background:"linear-gradient(90deg,rgba(201,168,76,0.4),transparent)"}}/>
          </div>
          <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:15,color:cream,margin:0,lineHeight:1.85}}>
            {alignment}
          </p>
        </div>
        ...
        <div ref={cardRef} style={{background:"linear-gradient(160deg,#0c0818 0%,#100c1a 50%,#0a0614 100%)",border:"1px solid rgba(201,168,76,0.5)",borderRadius:18,padding:"28px 22px 24px",marginBottom:14,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{textAlign:"center",marginBottom:4,position:"relative"}}>
            <div style={{fontFamily:"Cinzel,serif",fontSize:22,fontWeight:700,color:gold,letterSpacing:3}}>
              &#x2726; Mystic Fortunes &#x2726;
            </div>
          ...
          <div style={{textAlign:"center",marginBottom:16,position:"relative"}}>
            {onOneLine ? (
              <div style={{fontFamily:"Cinzel,serif",fontSize:17,color:rose,letterSpacing:1,fontWeight:600}}>{name1} &amp; {name2}</div>
            ) : (
              <div style={{fontFamily:"Cinzel,serif",fontSize:17,color:rose,letterSpacing:1,fontWeight:600,lineHeight:1.6}}>
                <div>{name1}</div>
                <div style={{fontSize:13,color:gold,letterSpacing:3}}>and</div>
                <div>{name2}</div>
              </div>
            )}
          </div>
          ...
          <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:13,color:cream,margin:"0 0 20px",lineHeight:1.75,textAlign:"center"}}>
            "{insight}"
          </p>

          <div style={{textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
            <span style={{fontSize:16}}>&#x1F52E;</span>
            <span style={{fontFamily:"Cinzel,serif",fontSize:9,color:"rgba(201,168,76,0.7)",letterSpacing:3,textTransform:"uppercase"}}>MYSTICFORTUNES.AI</span>
          </div>

          <div style={{position:"absolute",bottom:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,#c9a84c,transparent)"}}/>
        </div>

        <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:13,color:muted,textAlign:"center",lineHeight:1.7,margin:"0 0 20px",padding:"0 8px"}}>
        ...
        <button onClick={shareConnection} disabled={capturing}
          style={{width:"100%",padding:"16px",fontSize:15,borderRadius:12,background:"linear-gradient(135deg,rgba(176,64,90,0.3),rgba(120,30,60,0.3))",border:"1px solid rgba(176,64,90,0.6)",color:"#e06080",fontFamily:"Cinzel,serif",letterSpacing:1.5,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:12,opacity:capturing?0.7:1}}>
          <span style={{fontSize:20}}>&#x2764;&#xFE0F;</span>
          {capturing ? "Creating..." : shared ? "Shared!" : "Share Your Connection"}
        </button>

        <button onClick={onBack}
          style={{width:"100%",padding:"12px",fontSize:13,borderRadius:12,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",color:muted,fontFamily:"Cinzel,serif",letterSpacing:1,cursor:"pointer"}}>
          Return to Reading
        </button>
