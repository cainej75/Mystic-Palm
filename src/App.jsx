    ...
    {["Scan","Read","Reveal"].map((s,i)=>(
      <div key={s} style={{display:"flex",alignItems:"center",gap:6}}>
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          <div style={{width:22,height:22,borderRadius:"50%",background:i<=current?(i===2?C.rose:C.gold):C.border,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontFamily:"Cinzel,serif",color:i<=current?"#080510":C.muted,fontWeight:700,transition:"all 0.3s"}}>{i+1}</div>
          <span style={{fontFamily:"Cinzel,serif",fontSize:9,color:i<=current?(i===2?"#ff9db8":C.gold):C.muted,letterSpacing:1,textTransform:"uppercase",fontWeight:i===2&&i<=current?700:400}}>{s}</span>
        </div>
...
function ShareCard({reading, onOpenFullscreen, fullPaid}) {
  return (
    <div style={{textAlign:"center"}}>
      <button onClick={onOpenFullscreen} style={{background:`linear-gradient(135deg,${fullPaid?C.gold:C.rose},${fullPaid?"#7a6530":C.roseDim})`,border:"none",color:"white",borderRadius:8,padding:"12px 40px",fontSize:12,fontFamily:"Cinzel,serif",cursor:"pointer",fontWeight:600,letterSpacing:1,textTransform:"uppercase"}}>
        ✨ Generate Share Card
      </button>
    </div>
  );
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"linear-gradient(160deg,rgba(8,3,20,0.98) 0%,rgba(40,8,25,0.98) 50%,rgba(8,3,20,0.98) 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:1000}}>
      ...
      <div ref={shareCardRef} style={{maxWidth:280,width:"100%",background:"#080510",border:"2px solid #c9a84c",borderRadius:14,padding:"16px",boxShadow:"0 0 40px rgba(201,168,76,0.3)",animation:"slideUp 0.5s ease"}}>
        ...
        <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:"#c9a84c",letterSpacing:1,textAlign:"center"}}>🔮 mysticfortunes.ai</div>
...
function DetailsPage({ onSubmit, onBack }) {
  ...
  return (
    <div style={{paddingTop:40,paddingBottom:60,animation:"fadeUp 0.6s ease both"}}>
      ...
      <div style={{marginBottom:28}}>
        ...
        <input
          ...
          style={{
            ...
            border:`1px solid ${name.trim() ? C.gold+"99" : C.border}`,
            borderRadius:12,
            padding:"15px 18px",
            fontFamily:"IM Fell English,serif",
            fontStyle:"italic",
      ...
      <button
        onClick={onBack}
        style={{width:"100%",padding:"11px",fontSize:12,borderRadius:10,marginTop:14,background:"transparent",border:`1px solid ${C.border}`,color:C.muted,fontFamily:"Cinzel,serif",letterSpacing:1,cursor:"pointer"}}>
        ← Back
      </button>
    </div>
  );
        ...
        {items.map((item, i) => (
          <div
            ...
            style={{
              ...
              fontFamily: "Cinzel,serif",
              fontSize: 14,
              color: i === selectedIdx ? C.gold : C.muted,
...
function PartnerCompatibilityResult({ reading, userName, partnerName, onBack }) {
  const cardRef = React.useRef(null);
  const [shared, setShared] = React.useState(false);
  const [capturing, setCapturing] = React.useState(false);
  const [innerTab, setInnerTab] = React.useState("reading");

  const C = {
    bg:"#080510", surface:"#100c1a", border:"#2e1f40",
    gold:"#c9a84c", rose:"#b0405a", cream:"#e8d5b8", muted:"#6a5870",
  };
  ...
  const namesOnOneLine = (name1 + " & " + name2).length <= 26;
    ...
    try {
      ...
      canvas.toBlob(async (blob)=>{
        ...
        if (navigator.canShare && navigator.canShare({files:[file]})) {
          ...
          const a = document.createElement("a");
          a.download="cosmic-connection.png"; a.href=canvas.toDataURL("image/png"); a.click();
        }
      },"image/png");
  ...
  const tabs = [["extras","🃏","Today's Fortune"],["reading","💖","Partner Compatibility"],["lines","💎","Full Revelation"]];

  return (
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"#080510",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
    <div style={{background:"red",color:"white",padding:16,fontSize:14,fontFamily:"monospace",position:"sticky",top:0,zIndex:10}}>
      PARTNER RESULT PAGE LOADED<br/>
      score={reading?.score} userName={userName} partnerName={partnerName}
    </div>
    <div style={{maxWidth:480,margin:"0 auto",padding:"20px 16px 80px",animation:"fadeIn 0.4s ease both"}}>

      {/* Tabs */}
      <div style={{display:"flex",marginBottom:24,gap:6}}>
        {tabs.map(([id,icon,label])=>(
          <button key={id} onClick={()=>setInnerTab(id)}
            style={{flex:1,padding:"10px 4px",background:innerTab===id?`${C.gold}22`:"rgba(201,168,76,0.06)",
              border:`1px solid ${innerTab===id?C.gold:"rgba(201,168,76,0.25)"}`,borderRadius:10,
              color:innerTab===id?C.gold:C.muted,fontFamily:"Cinzel,serif",fontSize:10,
      ...
      <div style={{textAlign:"center",marginBottom:20}}>
        <div style={{fontSize:52,marginBottom:12,filter:"drop-shadow(0 0 16px rgba(176,64,90,0.6))"}}>❤️</div>
        <h2 style={{fontFamily:"Cinzel,serif",fontSize:26,fontWeight:700,color:C.gold,
          margin:"0 0 8px",letterSpacing:2,textShadow:`0 0 20px rgba(201,168,76,0.4)`}}>
          Your Cosmic Connection
        </h2>
        <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:15,
          color:C.muted,margin:0,letterSpacing:0.5}}>
          The universe has unveiled your alignment
        </p>
      </div>

      {/* Harmony Score Box */}
      <div style={{background:"rgba(201,168,76,0.06)",border:`1px solid ${C.gold}55`,
        borderRadius:16,padding:"24px 20px",marginBottom:20}}>
        <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:C.gold,letterSpacing:4,
          textTransform:"uppercase",textAlign:"center",marginBottom:16}}>✦ Harmony Score ✦</div>
        <div style={{textAlign:"center",marginBottom:16}}>
          <span style={{fontFamily:"Cinzel,serif",fontSize:64,fontWeight:700,
            color:C.rose,lineHeight:1,textShadow:`0 0 30px rgba(176,64,90,0.5)`}}>
            {score}%
          </span>
        </div>
        <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:15,
          color:C.cream,margin:0,lineHeight:1.8,textAlign:"center"}}>
          {insight}
        </p>
      </div>
      ...
      <div style={{marginBottom:24}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          <div style={{flex:1,height:1,background:`linear-gradient(90deg,transparent,${C.gold}44)`}}/>
          <span style={{fontFamily:"Cinzel,serif",fontSize:11,color:C.gold,letterSpacing:3,textTransform:"uppercase"}}>Your Alignment</span>
          <div style={{flex:1,height:1,background:`linear-gradient(90deg,${C.gold}44,transparent)`}}/>
        </div>
        <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:15,
          color:C.cream,margin:0,lineHeight:1.85}}>
          {alignment}
        </p>
      </div>
      ...
      <div ref={cardRef} style={{
        ...
        border:`1px solid ${C.gold}66`,borderRadius:18,padding:"28px 22px 24px",
        ...
        <div style={{position:"absolute",inset:0,pointerEvents:"none",overflow:"hidden"}}>
          {[...Array(16)].map((_,i)=>(
            <div key={i} style={{position:"absolute",
              left:`${(i*19+11)%93}%`,top:`${(i*27+7)%91}%`,
              width:i%3===0?2:1,height:i%3===0?2:1,borderRadius:"50%",
              background:i%2===0?"rgba(201,168,76,0.3)":"rgba(180,140,255,0.25)"}}/>
          ))}
        </div>

        {/* Card heading */}
        <div style={{textAlign:"center",marginBottom:4,position:"relative"}}>
          <div style={{fontFamily:"Cinzel,serif",fontSize:22,fontWeight:700,color:C.gold,
            letterSpacing:3,textShadow:`0 0 16px rgba(201,168,76,0.5)`}}>
            ✦ Mystic Fortunes ✦
          </div>
          <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:C.gold,letterSpacing:4,
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,margin:"14px 0"}}>
          <div style={{flex:1,height:1,background:`linear-gradient(90deg,transparent,${C.gold}44)`}}/>
          <span style={{color:C.rose,fontSize:18}}>❤️</span>
          <div style={{flex:1,height:1,background:`linear-gradient(90deg,${C.gold}44,transparent)`}}/>
        </div>
        ...
        <div style={{textAlign:"center",marginBottom:16,position:"relative"}}>
          {namesOnOneLine ? (
            <div style={{fontFamily:"Cinzel,serif",fontSize:17,color:C.rose,
              letterSpacing:1,fontWeight:600}}>
              {name1} & {name2}
            </div>
          ) : (
            <div style={{fontFamily:"Cinzel,serif",fontSize:17,color:C.rose,
              letterSpacing:1,fontWeight:600,lineHeight:1.6}}>
              <div>{name1}</div>
              <div style={{fontSize:13,color:C.gold,letterSpacing:3}}>and</div>
              <div>{name2}</div>
            </div>
          )}
        </div>
        ...
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontFamily:"Cinzel,serif",fontSize:9,color:C.gold,
            letterSpacing:3,textTransform:"uppercase",marginBottom:4}}>Date</div>
          <div style={{fontFamily:"Cinzel,serif",fontSize:12,color:C.cream}}>{dateStr}</div>
        </div>

        {/* Score */}
        <div style={{textAlign:"center",marginBottom:4}}>
          <div style={{fontFamily:"Cinzel,serif",fontSize:52,fontWeight:700,
            color:C.rose,lineHeight:1,textShadow:`0 0 24px rgba(176,64,90,0.5)`}}>
            {score}%
          </div>
          <div style={{fontFamily:"Cinzel,serif",fontSize:10,color:C.gold,
            letterSpacing:3,textTransform:"uppercase",marginTop:4}}>Harmony Score</div>
        </div>

        {/* Divider */}
        <div style={{display:"flex",alignItems:"center",gap:8,margin:"14px 0"}}>
          <div style={{flex:1,height:1,background:`linear-gradient(90deg,transparent,${C.gold}33)`}}/>
          <span style={{color:C.gold,fontSize:10}}>✦</span>
          <div style={{flex:1,height:1,background:`linear-gradient(90deg,${C.gold}33,transparent)`}}/>
        </div>

        {/* Insight */}
        <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:13,
          color:C.cream,margin:"0 0 20px",lineHeight:1.75,textAlign:"center"}}>
          "{insight}"
        </p>
        ...
        <div style={{textAlign:"center",display:"flex",alignItems:"center",
          ...
          <span style={{fontFamily:"Cinzel,serif",fontSize:9,color:`${C.gold}99`,
            letterSpacing:3,textTransform:"uppercase"}}>MYSTICFORTUNES.AI</span>
        </div>
      ...
      {/* Lunar cycle note */}
      <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:13,
        color:C.muted,textAlign:"center",lineHeight:1.7,margin:"0 0 20px",padding:"0 8px"}}>
        Your cosmic alignment shifts with each lunar cycle. Return next month as the tides turn
        and new celestial patterns emerge — {nextMonth} may reveal a new dimension of your connection.
      </p>

      {/* Share button */}
      <button onClick={shareConnection} disabled={capturing}
        style={{width:"100%",padding:"16px",fontSize:15,borderRadius:12,
          background:"linear-gradient(135deg,rgba(176,64,90,0.3),rgba(120,30,60,0.3))",
          border:`1px solid rgba(176,64,90,0.6)`,color:"#e06080",
          fontFamily:"Cinzel,serif",letterSpacing:1.5,cursor:"pointer",
          display:"flex",alignItems:"center",justifyContent:"center",gap:10,
          marginBottom:16,opacity:capturing?0.7:1}}>
        <span style={{fontSize:20}}>❤️</span>
        {capturing?"Creating..." : shared?"✓ Shared!":"Share Your Connection"}
      </button>

      {/* Footer */}
      <div style={{display:"flex",justifyContent:"center",gap:16,paddingTop:20,
        borderTop:`1px solid ${C.border}`,flexWrap:"wrap"}}>
        <span style={{fontFamily:"Cinzel,serif",fontSize:9,color:C.muted,letterSpacing:2}}>
          ✦ MYSTICFORTUNES.AI ✦
        </span>
      </div>
    </div>
...
export default function MysticFortunes() {
  ...
  const [birthDate,setBirthDate]=useState({day:1,month:1,year:1990});
  ...
  const devSkipToResult = () => {
    ...
    if (tapCountRef.current >= 5) {
      ...
      // Use current timestamp as seed so every dev tap gives a different reading
      const devSeed = (Date.now() & 0xFFFFFFFF) >>> 0;
      const devNames = ["Alice","Marcus","Sofia","James","Luna","Theo","Aria","Kai","Nora","Finn"];
      const devName  = devNames[Math.floor(Math.random() * devNames.length)];
      const devDob   = {day: Math.ceil(Math.random()*28), month: Math.ceil(Math.random()*12), year: 1970 + Math.floor(Math.random()*40)};
...
Respond ONLY with valid JSON, no markdown: {"greeting":"dramatic 1-sentence max 18 words","teaser":"2-3 sentence teaser with hook","lifeLine":"2 vivid sentences","heartLine":"2 vivid sentences","headLine":"2 vivid sentences","fateLine":"2 vivid sentences","fullReading":"4 paragraphs with \\n\\n","luckyNumber":"1-9","luckyColor":"evocative colour","soulSign":"mystical archetype 2-4 words","warning":"one ominous sentence","gift":"one sentence","pastLife":"one past life hint","nearFuture":"one 90-day prediction"}`;
  ...
  return(
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(0,0,0,0.9)",color:"#0f0",fontFamily:"monospace",fontSize:11,padding:"8px",zIndex:99999,pointerEvents:"none"}}>
      partnerPhase={partnerPhase||"null"} | hasResult={partnerReadingResult?"YES":"NO"} | partnerPaid={partnerPaid?"YES":"NO"} | phase={phase}
    </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"0 16px 90px",position:"relative",zIndex:2}}>
        ...
        {phase==="intro"&&(
          <div style={{paddingTop:36,paddingBottom:0,animation:"fadeUp 0.8s ease both"}}>
            ...
            {/* Play / Stop Intro buttons below video */}
            <div style={{display:"flex",gap:10,margin:"12px 0 0",justifyContent:"center"}}>
              <button
                onClick={()=>{
                  const v = videoRef2 && videoRef2.current;
                  if (!v) return;
                  if (muted) {
                    v.muted=false; v.currentTime=0; v.loop=false; v.play();
                    setMuted(false);
                    v.onended=()=>{ setMuted(true); v.loop=true; v.muted=true; v.play(); };
                  } else {
                    v.muted=true; v.loop=true; v.onended=null; setMuted(true);
                  }
                }}
                style={{background:"rgba(8,5,16,0.85)",border:`1.5px solid ${C.gold}88`,borderRadius:22,padding:"9px 22px",fontFamily:"Cinzel,serif",fontSize:16,color:C.gold,letterSpacing:1.5,cursor:"pointer",display:"flex",alignItems:"center",gap:8,textTransform:"uppercase",animation:muted?"introPulse 2s ease-in-out infinite":"none",boxShadow:muted?`0 0 12px ${C.gold}44`:`0 0 14px ${C.gold}77`}}>
                <span>{muted?"▶":"⏹"}</span>
            ...
            {/* Footer Links */}
            <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:30,paddingTop:20,borderTop:`1px solid ${C.border}`,flexWrap:"wrap"}}>
            ...
            */}
            ...
            {!partnerPhase&&activeTab==="reading"&&(
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                {showPartnerDetailsPage&&!partnerPaid?(
                  ...
                  <div style={{animation:"fadeIn 0.3s ease both"}}>
                    ...
                    <div style={{marginBottom:24}}>
                      ...
                      <div style={{display:"flex",gap:8}}>
                        <select value={partnerBirthDate.month} onChange={(e)=>setPartnerBirthDate({...partnerBirthDate,month:Number(e.target.value)})} style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,color:C.cream,fontFamily:"Crimson Text,serif",fontSize:14,padding:"10px",borderRadius:8,outline:"none"}}>
                          {Array.from({length:12},(_, i)=><option key={i+1} value={i+1}>{["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][i]}</option>)}
                        </select>
                        <select value={partnerBirthDate.day} onChange={(e)=>setPartnerBirthDate({...partnerBirthDate,day:Number(e.target.value)})} style={{flex:0.6,background:C.surface,border:`1px solid ${C.border}`,color:C.cream,fontFamily:"Crimson Text,serif",fontSize:14,padding:"10px",borderRadius:8,outline:"none"}}>
                          {Array.from({length:31},(_, i)=><option key={i+1} value={i+1}>{i+1}</option>)}
                        </select>
                        <select value={partnerBirthDate.year} onChange={(e)=>setPartnerBirthDate({...partnerBirthDate,year:Number(e.target.value)})} style={{flex:0.8,background:C.surface,border:`1px solid ${C.border}`,color:C.cream,fontFamily:"Crimson Text,serif",fontSize:14,padding:"10px",borderRadius:8,outline:"none"}}>
                          {Array.from({length:100},(_, i)=>{const yr=2024-i;return <option key={yr} value={yr}>{yr}</option>})}
                        </select>
                      </div>
                    </div>
                    ...
                    {/* Proceed Button */}
                    <button onClick={()=>{setShowPartnerDetailsPage(false);setPartnerPhase("capture");}} className="gold-btn" style={{width:"100%",padding:"14px",fontSize:16,borderRadius:11}}>
                  ...
                  <div style={{animation:"fadeIn 0.3s"}}>
                    ...
                    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px",marginBottom:20}}>
                      <div style={{marginBottom:14}}>
                        <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.gold,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>✨ What You'll Receive</div>
                        <p style={{fontFamily:"Crimson Text,serif",fontSize:15,color:C.cream,margin:0,lineHeight:1.7}}>A compatibility score revealing your romantic alignment · Deep insights into your relationship dynamics · Mystical guidance for your connection</p>
                      </div>
                      <div>
                        <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.rose,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>💝 Beautiful Share Card</div>
                    ...
                    <div style={{background:C.bg,border:`2px solid ${C.gold}`,borderRadius:14,padding:"16px",marginBottom:12,textAlign:"center",position:"relative",overflow:"hidden",maxWidth:280,margin:"0 auto 12px"}}>
                      ...
                      <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:C.gold,letterSpacing:1}}>🔮 mysticfortunes.ai</div>
                    ...
                    <div style={{background:"linear-gradient(135deg,#100616,#0c0812)",border:`1px solid ${C.gold}38`,borderRadius:14,padding:"18px 16px",textAlign:"center",animation:"orbGlow 4s ease-in-out infinite"}}>
                      <h3 style={{fontFamily:"Cinzel,serif",fontSize:18,color:C.gold,margin:"0 0 12px",letterSpacing:1}}>Unlock Your Couple's Destiny</h3>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center",justifyContent:"center",margin:"12px 0 16px"}}>
                        <span style={{fontFamily:"Cinzel,serif",fontSize:16,color:"#c090ff",textDecoration:"line-through"}}>$4.99</span>
                        <span style={{fontFamily:"Cinzel,serif",fontSize:36,color:C.gold,fontWeight:700}}>$2.99</span>
                      </div>
                      <button className="gold-btn" onClick={()=>setShowPartnerDetailsPage(true)} style={{width:"100%",padding:"14px",fontSize:16,borderRadius:11}}>💝 Unlock Partner Reading — $2.99</button>
                    </div>
                        </>
                      );
                    })()}
            ...
            {!partnerPhase&&activeTab==="lines"&&(
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                ...
                <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px",marginBottom:8}}>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.teal,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>✨ Your Gift</div>
                  {(fullPaid||subscribed)?
                ...
                {/* Heed This — paywalled */}
                <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px",marginBottom:8}}>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.rose,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>⚠ Heed This</div>
                  {(fullPaid||subscribed)?
                ...
                <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px",marginBottom:8}}>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:13,color:C.gold,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>✦ Madame La Voyante Speaks</div>
                  {(fullPaid||subscribed)?
                ...
                {!fullPaid&&!subscribed&&(
                  <div style={{background:"linear-gradient(135deg,#100616,#0c0812)",border:`1px solid ${C.gold}38`,borderRadius:14,padding:"18px 16px",marginBottom:10,textAlign:"center",animation:"orbGlow 4s ease-in-out infinite"}}>
                    ...
                    <p style={{fontFamily:"Cinzel,serif",color:"#c090ff",fontSize:14,lineHeight:1.7,margin:"0 0 10px",fontWeight:500,letterSpacing:0.5}}>Full palm analysis · Your Gift · Heed This · Past life echo · The next 30 days · Complete mystical narrative</p>
                    ...
                    <button className="gold-btn" onClick={()=>setFullPaid(true)} style={{width:"100%",padding:"12px",fontSize:16,borderRadius:11,marginBottom:6}}>🔮 Unlock Full Reading — $3.99</button>
                  </div>
                )}
              </div>
            )}
                    ...
                    return (
                      <div>
                        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",marginBottom:10}}>
                          <div style={{fontFamily:"Cinzel,serif",fontSize:17,color:C.gold,letterSpacing:3,textTransform:"uppercase"}}>✦ Today's Fortune ✦</div>
                          <div style={{fontFamily:"Cinzel,serif",fontSize:11,color:"#c9a84c",letterSpacing:1,marginTop:6}}>{dateStr}</div>
                        </div>
                      </div>
                    );
                  })()}
                ...
                {(()=>{
                  ...
                  return (
                    <div style={{background:C.surface,border:`1px solid ${C.gold}44`,borderRadius:14,padding:"16px",marginBottom:12,position:"relative",overflow:"hidden"}}>
                      <div style={{position:"absolute",top:0,left:0,right:0,height:1.5,background:`linear-gradient(90deg,transparent,${C.gold},transparent)`}}/>
                      <div style={{textAlign:"center",marginBottom:12}}>
                        <span style={{fontFamily:"Cinzel,serif",fontSize:15,color:C.gold,letterSpacing:3,textTransform:"uppercase"}}>✦ {MONTHS_LBL[curMonth]} Elements ✦</span>
                        <p style={{fontFamily:"Crimson Text,serif",fontStyle:"italic",fontSize:15,color:C.cream,margin:"6px 0 0",lineHeight:1.5,opacity:0.85}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                        {[["🌙",mSign,"Soul Sign"],["🔢",mNum,"Lucky #"],["🎨",mColor,"Colour"]].map(([ic,v,l])=>(
                          <div key={l} style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${C.gold}22`,borderRadius:10,padding:"10px 7px",textAlign:"center"}}>
                            ...
                            <div style={{fontFamily:"Cinzel,serif",fontSize:9,color:C.cream,letterSpacing:1,marginTop:2,opacity:0.85}}>{l}</div>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        {phase==="result"&&reading&&!partnerPhase&&(
          <>
            <Divider/>
            {activeTab==="extras"&&(
              <button onClick={()=>{setPhase("intro");setReading(null);setPalmImage(null);setPartnerPaid(false);setFullPaid(false);setSubscribed(false);setSpeaking(false);}}
                style={{width:"100%",padding:"12px",fontSize:14,borderRadius:9,background:"rgba(148,90,210,0.18)",border:"1px solid rgba(148,90,210,0.6)",color:"#c090ff",fontFamily:"Cinzel,serif",letterSpacing:1,cursor:"pointer",marginTop:16}}>
                🔄 Begin a New Reading
              </button>
            )}
            {(activeTab==="reading"||activeTab==="lines")&&(
      ...
      {partnerPalmError&&(
        <div style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(8,5,16,0.99)",display:"flex",alignItems:"center",justifyContent:"center",padding:"0 16px"}}>
          <div style={{maxWidth:420,width:"100%",textAlign:"center",position:"relative",zIndex:1}}>
            ...
            <h2 style={{fontFamily:"Cinzel,serif",fontSize:"clamp(22px,6vw,32px)",color:C.gold,margin:"0 0 6px",letterSpacing:2,animation:"goldGlow 3s ease-in-out infinite"}}>Madame La Voyante</h2>
            <h3 style={{fontFamily:"Cinzel,serif",fontSize:"clamp(16px,4vw,22px)",color:C.rose,margin:"0 0 18px",fontWeight:400,letterSpacing:1}}>Cannot See Their Palm</h3>
            <div style={{display:"flex",alignItems:"center",gap:10,margin:"0 0 22px"}}>
              <div style={{flex:1,height:1,background:`linear-gradient(90deg,transparent,${C.border})`}}/>
              <span style={{color:C.rose,fontSize:14}}>✦</span>
              <div style={{flex:1,height:1,background:`linear-gradient(90deg,${C.border},transparent)`}}/>
            ...
            <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",color:C.cream,fontSize:"clamp(17px,4vw,20px)",lineHeight:1.8,margin:"0 0 14px",opacity:0.95}}>
              A clear image of their <span style={{color:C.rose}}>left palm facing upward</span> is required — so the ancient lines of the hand are fully visible.
            </p>
            <p style={{fontFamily:"Crimson Text,serif",fontStyle:"italic",color:C.cream,fontSize:"clamp(15px,3.5vw,17px)",lineHeight:1.7,margin:"0 0 26px",opacity:0.8}}>
            ...
            <div style={{background:"rgba(201,168,76,0.09)",border:`1px solid ${C.gold}44`,borderRadius:12,padding:"14px 18px",marginBottom:26,display:"flex",alignItems:"center",gap:14,textAlign:"left"}}>
              <span style={{fontSize:30,flexShrink:0}}>☝️</span>
              <p style={{fontFamily:"Crimson Text,serif",fontSize:16,color:C.cream,margin:0,lineHeight:1.6,opacity:0.88}}>
                Have them hold their <strong>left hand</strong> with the palm facing the camera, fingers spread open, in good lighting.
              </p>
            </div>
      ...
      {phase==="result"&&reading&&!partnerPhase&&(
        <div style={{display:"flex",justifyContent:"center",gap:16,paddingTop:16,paddingBottom:60,borderTop:`1px solid ${C.border}`,flexWrap:"wrap"}}>
          <Link to="/privacy-policy" style={{fontFamily:"Cinzel,serif",fontSize:10,color:C.gold,letterSpacing:1,textDecoration:"none",transition:"all 0.3s"}} onMouseEnter={(e)=>e.target.style.color=C.rose} onMouseLeave={(e)=>e.target.style.color=C.gold}>Privacy Policy</Link>
          <span style={{color:C.border}}>•</span>
          <Link to="/terms-and-conditions" style={{fontFamily:"Cinzel,serif",fontSize:10,color:C.gold,letterSpacing:1,textDecoration:"none",transition:"all 0.3s"}} onMouseEnter={(e)=>e.target.style.color=C.rose} onMouseLeave={(e)=>e.target.style.color=C.gold}>Terms & Conditions</Link>
        </div>
      )}
