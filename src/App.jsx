...
function PartnerCompatibilityResult({ reading, userName, partnerName, nameDisplayOrder, onBack, setActiveTab, pdfPaid, pdfGenerating, pdfNarrative, onPDFPayment, birthDate, partnerBirthDate, soulmateAnswers, soulmateJustCurious }) {
  ...
  // Auto-switch to deep tab when narrative finishes generating after payment
  useEffect(() => {
    if (pdfNarrative && pdfPaid) {
      const t = setTimeout(() => setActiveResultTab("deep"), 300);
      return () => clearTimeout(t);
    }
  }, [pdfNarrative, pdfPaid]);
    ...
    const verifyPaymentOnReturn = async () => {
      ...
      if (sessionId.startsWith('free_')) {
        ...
        if (productType === 'full_revelation') {
          ...
          // narrative generation triggers via useEffect on partnerPDFPaid
        }
        window.history.replaceState({}, document.title, window.location.pathname);
        return;
      }
...
{"greeting":"${userName}, [2-3 sentences weaving palm impression with celestial signature]","lifeLine":"[3-4 sentences — life path energy, stamina, reinvention, Chinese zodiac influence if relevant]","heartLine":"[3-4 sentences — emotional patterns, love nature, moon sign resonance]","headLine":"[3-4 sentences — mental style, intuition, ruling planet influence]","fateLine":"[3-4 sentences — soul-level destiny direction, life path number significance, what is being built over years not weeks]","fullReading":"[4 paragraphs separated by \n\n: 1.Overall cosmic portrait. 2.The central challenge written in both palm and stars. 3.What is approaching in their life. 4.The unique gift only they carry.]","nearFuture":"[2-3 sentences — specific 30-90 day celestial and palm prediction, distinct from fateLine]"}`;
  ...
  const generateSoulmateNarrative = async (answers, justCurious, uName, uDob, pName, pDob) => {
    ...
    const userMeta  = ZODIAC_META_LOCAL[userSign]  || {element:"Earth",modality:"Fixed"};
    const partMeta  = ZODIAC_META_LOCAL[partSign]   || {element:"Water",modality:"Mutable"};
...
Respond ONLY with valid JSON, no preamble, no markdown:
{
  ...
  // Auto-switch to deep tab when narrative finishes so user sees the reading
  useEffect(() => {
    if (soulmateNarrative && partnerPhase === 'result') {
      // Small delay so the tab switch feels intentional, not jarring
      const t = setTimeout(() => {
        // Only switch if we're on the PartnerCompatibilityResult screen
        // The activeTab here is the main App tab ("reading"), not the inner result tab
        // The inner tab switch is handled inside PartnerCompatibilityResult via prop
      }, 400);
      return () => clearTimeout(t);
    }
  }, [soulmateNarrative, partnerPhase]);
  ...
  return(
    ...
    {partnerPhase==="result"&&partnerReadingResult&&(
      <>
        {/* Full-screen loading overlay during deeper connection narrative generation */}
        {soulmateNarrativeGenerating&&(()=>{
          const NarrativeLoadingScreen=()=>{
            const msgs=["Madame Zafira is writing your reading…","Consulting your celestial charts…","Weaving the stars into words…","The cosmos cannot be rushed…","Your deeper connection is being revealed…"];
            const [msgIdx,setMsgIdx]=React.useState(0);
            React.useEffect(()=>{const t=setInterval(()=>setMsgIdx(i=>(i+1)%msgs.length),1400);return()=>clearInterval(t);},[]);
            return(
              <div style={{position:"fixed",inset:0,zIndex:10000,background:"linear-gradient(160deg,#060412 0%,#0d0635 50%,#060412 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:32}}>
                {/* Animated orb */}
                <div style={{position:"relative",width:160,height:160}}>
                  <div style={{position:"absolute",inset:0,borderRadius:"50%",background:"radial-gradient(circle at 40% 35%,#8b3fa0,#2a0d5e 60%,#0d0520)",boxShadow:"0 0 80px rgba(139,47,201,0.7),inset 0 0 50px rgba(192,144,255,0.15)",animation:"orbGlow 3s ease-in-out infinite"}}/>
                  <div style={{position:"absolute",top:"18%",left:"22%",width:"35%",height:"22%",borderRadius:"50%",background:"rgba(255,255,255,0.14)",filter:"blur(4px)",transform:"rotate(-20deg)"}}/>
                  <div style={{position:"absolute",inset:0,borderRadius:"50%",border:"2px solid rgba(201,168,76,0.5)",animation:"spin 8s linear infinite"}}/>
                  <div style={{position:"absolute",inset:6,borderRadius:"50%",border:"1px solid rgba(192,144,255,0.35)",animation:"spin 5s linear infinite reverse"}}/>
                ...
                <div style={{textAlign:"center",maxWidth:300,padding:"0 24px"}}>
                  <div style={{fontFamily:"Cinzel,serif",fontSize:15,color:"#c9a84c",letterSpacing:2.5,textTransform:"uppercase",animation:"auraPulse 2s ease-in-out infinite",marginBottom:12}}
                    key={msgIdx}>{msgs[msgIdx]}</div>
                  <div style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:15,color:"rgba(192,144,255,0.7)",lineHeight:1.6}}>
                    "The stars speak slowly to those who deserve their secrets…"
                  </div>
                  <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:20}}>
                    {[0,1,2].map(i=>(<div key={i} style={{width:7,height:7,borderRadius:"50%",background:"#c090ff",opacity:0.4,animation:`auraPulse 1.4s ease-in-out ${i*0.45}s infinite`}}/>))}
                  </div>
                </div>
              </div>
            );
          };
          return <NarrativeLoadingScreen/>;
        })()}
