    ...
    const verifyPaymentOnReturn = async () => {
      ...
      if (sessionId.startsWith('free_')) {
        ...
        if (productType === 'full_revelation') {
          ...
          const savedReading = sessionStorage.getItem('pendingPartnerReading');
          const savedUserName = sessionStorage.getItem('pendingUserName');
          const savedPartnerName = sessionStorage.getItem('pendingPartnerName');
          if (savedReading) {
            setPartnerReadingResult(JSON.parse(savedReading));
            if (savedUserName) setUserName(savedUserName);
            if (savedPartnerName) setPartnerName(savedPartnerName);
            setPartnerPhase('result');
            sessionStorage.removeItem('pendingPartnerReading');
            sessionStorage.removeItem('pendingUserName');
            sessionStorage.removeItem('pendingPartnerName');
          }
        }
        showToast('Complimentary reading unlocked!');
      ...
      try {
        ...
        if (data.valid) {
          ...
          if (data.productType === 'full_revelation') {
            ...
            setPartnerPaid(true);
            setDownloadType('partner_compatibility');
            // Restore reading from sessionStorage so results page shows correctly
            const savedReading = sessionStorage.getItem('pendingPartnerReading');
            const savedUserName = sessionStorage.getItem('pendingUserName');
            const savedPartnerName = sessionStorage.getItem('pendingPartnerName');
            if (savedReading) {
              const restored = JSON.parse(savedReading);
              setPartnerReadingResult(restored);
              if (savedUserName) setUserName(savedUserName);
              if (savedPartnerName) setPartnerName(savedPartnerName);
              setPartnerPhase('result');
              sessionStorage.removeItem('pendingPartnerReading');
...
Respond ONLY with valid JSON, no markdown: {"greeting":"dramatic 1-sentence max 18 words","teaser":"2-3 sentence teaser with hook","lifeLine":"2 vivid sentences","heartLine":"2 vivid sentences","headLine":"2 vivid sentences","fateLine":"2 vivid sentences","fullReading":"4 paragraphs with \\n\\n","luckyNumber":"1-9","luckyColor":"evocative colour","soulSign":"mystical archetype 2-4 words","warning":"one ominous sentence","gift":"one sentence","pastLife":"one past life hint","nearFuture":"one 90-day prediction"}`;
  ...
  const handlePartnerPayment = () => {
    // Save reading state so we can restore it after Stripe redirect
    if (partnerReadingResult) {
      sessionStorage.setItem('pendingPartnerReading', JSON.stringify(partnerReadingResult));
    }
    if (userName) sessionStorage.setItem('pendingUserName', userName);
    if (partnerName) sessionStorage.setItem('pendingPartnerName', partnerName);
