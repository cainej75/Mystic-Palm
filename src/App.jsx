...
const FullScreenShareCard = React.memo(function FullScreenShareCard({reading, userName, birthDate, onClose, todayFortune, onSaved}) {
  ...
  if (showScreenshot) {
    const [showOpenBrowser, setShowOpenBrowser] = React.useState(false);
    if (showOpenBrowser) return <OpenInBrowserOverlay onDismiss={()=>setShowOpenBrowser(false)} subtitle="Open mysticfortunes.ai in your browser to download your share card."/>;
    return (
                ...
                <button
                  onClick={async()=>{
                    try {
                      ...
                      if(isInAppBrowser()){
                        const result=await generatePartnerCompatibilityPDF(reading, name1, name2, score, null, pdfNarrative, soulmateAnswers, soulmateJustCurious, true);
                        setPdfLongPressUrl({url:result.blobUrl,filename:result.filename,type:'partner'});
                      } else {
...
Respond ONLY with valid JSON, no preamble, no markdown:
{
  ...
  return(
    <>
    {/* PDF download overlay for in-app browsers */}
    {pdfLongPressUrl&&(
      <OpenInBrowserOverlay
        onDismiss={()=>{
          setTimeout(()=>URL.revokeObjectURL(pdfLongPressUrl.url),5000);
          setPdfLongPressUrl(null);
        }}
        subtitle="Open mysticfortunes.ai in your browser to download your PDF."
      />
    )}
    {showNatalScreen&&(
      <NatalInfoScreen
                  ...
                  <button
                    onClick={async()=>{
                      ...
                      try{
                        ...
                        if(isInAppBrowser()){
                          const result=await generateFullRevelationPDF(reading,userName,birthDate,true);
                          setPdfLongPressUrl({url:result.blobUrl,filename:result.filename,type:'full'});
                        } else {
