...
export default function MysticFortunes() {
  ...
  useEffect(()=>{
    if(phase==="biometric" && palmImage && palmLandmarks) {
      const valid = validatePalmImage(palmLandmarks);
      if (!valid) {
        setPalmError(true);
        setPhase("capture");
      }
    }
  },[phase,palmImage,palmLandmarks]);

  // Validate partner palm silently when captured
  useEffect(()=>{
    if(partnerPhase==="biometric" && partnerPalmImage && partnerPalmLandmarks) {
      const valid = validatePalmImage(partnerPalmLandmarks);
      if (!valid) {
        setPartnerPalmError(true);
        setPartnerPhase("capture");
      }
    }
  },[partnerPhase,partnerPalmImage,partnerPalmLandmarks]);
      ...
      {partnerPhase&&(
        <div style={{paddingTop:28,paddingBottom:80,animation:"fadeUp 0.6s ease both"}}>
          ...
          {partnerPhase==="result"&&partnerReadingResult&&(
            <div style={{paddingTop:28,paddingBottom:80,animation:"fadeIn 0.6s ease both"}}>
              {/* Tab Buttons */}
              <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,marginBottom:12,gap:6}}>
                {[["extras","🃏","Today's Fortune"],["reading","💖","Partner Compatibility"],["lines","💎","Full Revelation"]].map(([id,icon,label])=>(
                  <button key={id} onClick={()=>{setActiveTab(id);}} className="result-tab"
                    style={{flex:1,background:id==="reading"?`${C.gold}22`:"rgba(201,168,76,0.08)",border:`1px solid ${id==="reading"?C.gold:"rgba(201,168,76,0.3)"}`,borderBottom:`3px solid ${id==="reading"?C.gold:"transparent"}`,color:id==="reading"?C.gold:"rgba(201,168,76,0.6)",fontFamily:"Cinzel,serif",fontSize:10,letterSpacing:1,textTransform:"uppercase",padding:"10px 8px",cursor:"pointer",transition:"all 0.3s ease",display:"flex",flexDirection:"column",alignItems:"center",gap:3,borderRadius:"8px 8px 0 0",boxShadow:id==="reading"?`0 0 15px ${C.gold}44`:"none",animation:id==="reading"?"tabPulse 2s ease-in-out infinite":"none",transform:"none"}}>
                    <span style={{fontSize:20,transition:"transform 0.3s ease"}}>{icon}</span>
                    <span style={{fontWeight:id==="reading"?700:500}}>{label}</span>
                  </button>
                ))}
              </div>
              ...
              {/* Back to Today's Fortune Button */}
              <button onClick={()=>{setPartnerPhase(null);setActiveTab("extras");}} style={{width:"100%",padding:"12px",fontSize:14,borderRadius:9,background:"rgba(148,90,210,0.18)",border:"1px solid rgba(148,90,210,0.6)",color:"#c090ff",fontFamily:"Cinzel,serif",letterSpacing:1,cursor:"pointer",marginTop:16}}>
                ← Back to Today's Fortune
              </button>
            </div>
          )}
