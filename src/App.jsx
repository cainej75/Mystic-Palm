...
function FullScreenShareCard({reading, userName, birthDate, onClose}) {
    ...
    try {
      ...
      script.onload = async () => {
        try {
          ...
          // Brighten the canvas image dramatically
          const ctx = canvas.getContext('2d');
          const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
          const data = imageData.data;
          
          // Massively increase brightness with multiplier AND brightness offset
          for (let i = 0; i < data.length; i += 4) {
            data[i] = Math.min(255, data[i] * 3.2 + 100);     // Red: 3.2x multiplier + 100 brightness
            data[i + 1] = Math.min(255, data[i + 1] * 3.2 + 100); // Green
            data[i + 2] = Math.min(255, data[i + 2] * 3.2 + 100); // Blue
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"linear-gradient(160deg,rgba(8,3,20,0.98) 0%,rgba(40,8,25,0.98) 50%,rgba(8,3,20,0.98) 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:1000}}>
      ...
      <div ref={shareCardRef} style={{maxWidth:420,width:"100%",background:C.bg,border:`3px solid ${C.gold}`,borderRadius:20,padding:"24px 20px",boxShadow:`0 0 40px rgba(201,168,76,0.3)`,animation:"slideUp 0.5s ease"}}>
        
        {/* Heading */}
        <div style={{textAlign:"center",marginBottom:20}}>
          <div style={{fontFamily:"IM Fell English,serif",fontSize:40,fontStyle:"italic",color:C.gold,marginBottom:6,letterSpacing:1}}>Mystic Fortunes</div>
        </div>
        
        {/* Person's Name */}
        <div style={{marginBottom:16,textAlign:"center"}}>
          <div style={{fontFamily:"Crimson Text,serif",fontSize:32,color:C.gold,fontStyle:"italic"}}>{userName}</div>
        </div>
          ...
          <div style={{paddingTop:28,paddingBottom:80,animation:"fadeUp 0.6s ease both"}}>
            ...
            */}
            ...
            <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,marginBottom:12}}>
              {[["extras","🃏","Today's Fortune"],["reading","💎","Your Gift / Warning"],["lines","🔒","Locked Insights"]].map(([id,icon,label])=>(
            ...
            {activeTab==="extras"&&(
              <div style={{animation:"fadeIn 0.3s ease both"}}>
                <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px",marginBottom:10,position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{fontFamily:"Cinzel,serif",fontSize:17,color:C.gold,letterSpacing:3,textTransform:"uppercase",marginBottom:10}}>✦ Today's Fortune ✦</div>
                  {(()=>{
                    const n = userName||"Seeker";
                    const dayOfYear = Math.floor((Date.now()-new Date(new Date().getFullYear(),0,0))/86400000);
                    const dailySeed = (generateSeed(userName,birthDate)^(dayOfYear*2654435761))>>>0;
                    ...
                    const todayFortune = DF[Math.floor(dRng()*DF.length)];
                    return <p style={{fontFamily:"IM Fell English,serif",fontStyle:"italic",fontSize:19,color:C.cream,margin:0,lineHeight:1.85}}>{`"${todayFortune}"`}</p>;
                  })()}
                </div>
                {(()=>{
                  const MONTHS_LBL = ["January","February","March","April","May","June","July","August","September","October","November","December"];
