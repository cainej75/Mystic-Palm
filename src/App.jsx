...
Respond ONLY with valid JSON, no markdown: {"greeting":"dramatic 1-sentence max 18 words","teaser":"2-3 sentence teaser with hook","lifeLine":"2 vivid sentences","heartLine":"2 vivid sentences","headLine":"2 vivid sentences","fateLine":"2 vivid sentences","fullReading":"4 paragraphs with \\n\\n","luckyNumber":"1-9","luckyColor":"evocative colour","soulSign":"mystical archetype 2-4 words","warning":"one ominous sentence","gift":"one sentence","pastLife":"one past life hint","nearFuture":"one 90-day prediction"}`;
  ...
  return(
        ...
        <FreezeFrame
          ...
          onDone={(valid)=>{
            if(partnerPhase==="freeze"){
              setDebugMsg(valid ? "Partner hand valid - going to biometric" : "Partner hand invalid - retry");
              if (!valid) { setPartnerPalmError(true); setPartnerPhase("capture"); }
              else setPartnerPhase("biometric");
            }else{
              setDebugMsg(valid ? "User hand valid - going to biometric" : "User hand invalid - retry");
        ...
        <BiometricScan
          ...
          onComplete={()=>{
            if(partnerPhase==="biometric"){
              setDebugMsg("Partner reading complete!");
              ...
              setDebugMsg("User reading complete!");
              getReading();
            }
          }}
        />
    ...
    <div style={{minHeight:"100vh",background:C.bg,position:"relative",overflow:"hidden"}}>
      ...
      {/* DEBUG DISPLAY */}
      <div style={{position:"fixed",top:20,left:20,background:"rgba(0,0,0,0.95)",border:"2px solid #ff6b9d",color:"#ff6b9d",padding:"12px 16px",borderRadius:8,fontFamily:"monospace",fontSize:11,zIndex:9999,maxWidth:350,lineHeight:1.6}}>
        <div>phase: {phase}</div>
        <div>partnerPhase: {partnerPhase}</div>
        <div>cameraOn: {cameraOn ? "YES" : "NO"}</div>
        <div>partnerCameraOn: {partnerCameraOn ? "YES" : "NO"}</div>
        <div>palmImage: {palmImage ? "YES" : "NO"}</div>
        <div>partnerPalmImage: {partnerPalmImage ? "YES" : "NO"}</div>
        <div>msg: {debugMsg}</div>
      </div>
        ...
        {(phase==="capture"&&cameraOn) || (partnerPhase==="capture"&&partnerCameraOn)?(
          <>
            <FullscreenCamera
              ...
              onCapture={(img,lm)=>{
                alert("CAPTURE TRIGGERED! partnerPhase=" + partnerPhase);
                setDebugMsg("onCapture: partnerPhase=" + partnerPhase);
