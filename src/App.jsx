...
{"greeting":"${userName}, [2-3 sentences weaving palm impression with celestial signature]","lifeLine":"[3-4 sentences — life path energy, stamina, reinvention, Chinese zodiac influence if relevant]","heartLine":"[3-4 sentences — emotional patterns, love nature, moon sign resonance]","headLine":"[3-4 sentences — mental style, intuition, ruling planet influence]","fateLine":"[3-4 sentences — soul-level destiny direction, life path number significance, what is being built over years not weeks]","fullReading":"[4 paragraphs separated by \n\n: 1.Overall cosmic portrait. 2.The central challenge written in both palm and stars. 3.What is approaching in their life. 4.The unique gift only they carry.]","nearFuture":"[2-3 sentences — specific 30-90 day celestial and palm prediction, distinct from fateLine]"}`;
  ...
  const generateSoulmateNarrative = async (answers, justCurious, uName, uDob, pName, pDob) => {
    ...
    if (!answers) {
      ...
      const ZODIAC_NAMES_FB = ["Capricorn","Aquarius","Pisces","Aries","Taurus","Gemini","Cancer","Leo","Virgo","Libra","Scorpio","Sagittarius","Capricorn"];
      const getSignFB = (dob) => { if (!dob) return "Unknown"; try { return ZODIAC_NAMES_FB[getZodiac(dob)?.index] || "Unknown"; } catch { return "Unknown"; } };
  ...
  return(
    ...
    {pdfLongPressUrl&&(
      <div style={{position:"fixed",inset:0,zIndex:99999,background:"rgba(0,0,0,0.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{fontFamily:"Cinzel,serif",fontSize:15,color:"#c9a84c",letterSpacing:1,marginBottom:8,textAlign:"center",fontWeight:700}}>Your PDF is ready</div>
        <div style={{fontFamily:"Crimson Text,serif",fontStyle:"italic",fontSize:16,color:"#e8d5b8",marginBottom:24,textAlign:"center",lineHeight:1.6}}>
          Tap the button below to open your PDF, then use your browser's share or save option to keep it.
        </div>
        <a href={pdfLongPressUrl.url} target="_blank" rel="noopener noreferrer"
          style={{display:"block",background:"linear-gradient(135deg,#c9a84c,#9d7d2e)",color:"#080510",fontFamily:"Cinzel,serif",fontSize:15,fontWeight:700,letterSpacing:1.5,padding:"16px 32px",borderRadius:12,textDecoration:"none",marginBottom:16,textAlign:"center"}}>
          📜 Open PDF
        </a>
        <button onClick={()=>{
          if(pdfLongPressUrl.type==='full') setFullPDFSaved(true);
          if(pdfLongPressUrl.type==='partner') setDeepPDFDownloaded(true);
          setTimeout(()=>URL.revokeObjectURL(pdfLongPressUrl.url),10000);
