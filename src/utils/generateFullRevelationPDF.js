...
export async function generateFullRevelationPDF(reading, userName, birthDate) {
  ...
  for(let i=0;i<allBlocks.length;i++){
    if(used+heights[i]<=P1_AVAIL){p1.push(allBlocks[i]);used+=heights[i];}
    else{p2=allBlocks.slice(i);break;}
  }
  if(!p2.length) p2=p1.splice(4);
  ...
  // Page 1: hero img tag followed by content in normal flow — one single canvas
  const page1Html = `
  ...
  const [c1, c2] = await Promise.all([
    ...
    renderPageToCanvas(page2Html, PAGE_W, PAGE_H),
  ]);
  ...
  pdf.addImage(c1.toDataURL('image/jpeg',0.93), 'JPEG', 0, 0, pdfW, pdfH);
  pdf.addPage();
  pdf.addImage(c2.toDataURL('image/jpeg',0.93), 'JPEG', 0, 0, pdfW, pdfH);
