...
export async function generateFullRevelationPDF(reading, userName, birthDate, returnBlob=false) {
  ...
  const filename = `Mystic-Fortunes-${name.replace(/\s+/g,'-')}-${dateStr.replace(/\s+/g,'-')}.pdf`;
  if (returnBlob) {
