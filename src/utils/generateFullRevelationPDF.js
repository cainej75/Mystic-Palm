...
export async function generateFullRevelationPDF(reading, userName, birthDate) {
  ...
  // PAGE 1 — header + individual blocks (heart, head, life, gift, warning, past life)
  const page1Html = `
  ...
  // PAGE 2 — The Coming Months + Your True Path (full reading narrative)
  const page2Html = `
    <style>${SHARED_STYLES}</style>
    <div class="page">
      <div class="block block-next30">
        <div class="block-label lbl-gold">🌟 &nbsp; The Coming Months</div>
        <div class="block-text">${reading?.nearFuture || ''}</div>
      </div>
      <div class="star-divider"><span>✦</span></div>
      <div class="block block-zafira">
        <div class="block-label lbl-gold">✦ &nbsp; Your True Path</div>
