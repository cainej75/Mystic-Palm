...
// Coupon configuration embedded here (not imported from frontend)
const COUPON_CONFIG = {
  SAVE1: {
    discountAmount: 1.00,
    active: true,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${process.env.VERCEL_URL || 'http://localhost:5173'}/success?session_id={CHECKOUT_SESSION_ID}&type=${productType}`,
      cancel_url: `${process.env.VERCEL_URL || 'http://localhost:5173'}/cancelled`,
      metadata: {
        productType,
